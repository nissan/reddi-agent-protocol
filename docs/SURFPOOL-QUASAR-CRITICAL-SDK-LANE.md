...
- Step assertions require evidence that is provably complete. Both stages report what they dropped — marker-replaced oversized records from redaction, omitted chunks from the bounded buffer — and any loss from either makes the step's evidence incomplete. Incomplete evidence is refused outright: the step fails naming the dropped character, chunk, and oversized-line counts and the log that holds the retained output, rather than certifying a "must not contain" boundary over whatever survived. Redaction and truncation shape what is displayed and logged; they never decide what an assertion is allowed to conclude. `SUMMARY.md` carries the same fact for the reader of a published receipt, and keeps the two channels apart because they lose output from different places: an `Evidence completeness` line reports the log's own loss (records replaced by an oversized-line marker, which never reached disk) separately from the bounded assertion/display spool's truncation (in-memory only — those characters were already written to the log, a recommendation the line makes only while the log's persistence is proven). A spool truncation is therefore never published as log omission. Each step's record is finalized on every terminal path — normal exit, nonzero exit, command timeout, SIGINT, and a failed log write alike — so a step that was aborted still discloses what it dropped rather than defaulting to zero. If appending to the log ever fails, the run stops claiming any count for it and reports the persisted log as indeterminate instead; a zero-loss claim is only ever published for a log every write reached. Steps that carry assertions cannot appear on either side — their evidence had to be complete to pass — so the line only ever names steps no boundary rests on.
