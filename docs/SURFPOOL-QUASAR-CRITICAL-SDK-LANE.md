...
Both fingerprint and artifact hashes are read descriptor-bound: each file is opened with `O_NOFOLLOW`, the opened descriptor is re-resolved through `/proc/self/fd` and required to land inside its allowed root, its device/inode/size must still match what the walk recorded, and the bytes are hashed from that same descriptor. A platform that cannot supply that proof — anything other than Linux, or a Linux without `/proc/self/fd` — is refused rather than downgraded to a path-following read, so publishing *and* verifying receipts is Linux-only.
