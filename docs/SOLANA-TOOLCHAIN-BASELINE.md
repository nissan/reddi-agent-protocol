...
4. Remove Anchor/AVM: `avm uninstall 1.1.2` if available, then remove the AVM Cargo binary if this setup installed it. To roll back specifically to the previous merged Anchor baseline, re-checkout the pre-upgrade pins (`Anchor.toml`, `programs/escrow/Cargo.toml`, `Cargo.lock`, and `config/toolchain/solana-baseline-assets.json`) before reinstalling `1.0.0` through the same verified AVM path.
