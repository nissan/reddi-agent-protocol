...
The two pre-remediation volunteer walkthroughs formerly surfaced on `/testers` are withheld: their players and direct public-serving files were removed. The written Ollama and OpenOnion setup guides remain available without presenting unreviewed recordings as current product evidence. What holds that is behavioral, not a search of the page source: the route scan in `e2e/public-claim-boundary.spec.ts` fails any gated route that renders a `<video>` without a caption track under `public/videos/onboarding/captions/`, so a recording returning to `/testers` under any other identifier or public path turns the lane red unless its narration is in the text the static gate reads.
