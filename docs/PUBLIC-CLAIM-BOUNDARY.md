...
That obligation is not yet met everywhere, and the exception is recorded here rather than left implicit. `/testers` renders two volunteer setup walkthroughs — `public/video/volunteers/reddi-ollama-x402-wrapper-guide-20260427.mp4` and `reddi-openonion-x402-wrapper-guide-20260427.mp4`, both committed 2026-04-27 — in `<video controls>` players with no boundary notice, and neither half of the gate can reach them: the static gate scans owned text only, and the DOM scan reads rendered copy, not a recording's frames or audio. They differ from the withheld captures in subject — wrapper setup against a volunteer's own deployment, rather than this repository's protocol, discovery, or economics copy — but that is a judgement from their titles and the written guides they link to, not a review of their frames. Treat them as the one known open item against the rule above: before any later release note supersedes this boundary they must be reviewed against current copy and then withheld, re-recorded, or given the same point-of-playback disclosure the `/tour` explainer carries.
