...
The two pre-remediation volunteer walkthroughs formerly surfaced on `/testers` are withheld: their players and direct public-serving files were removed. The written Ollama and OpenOnion setup guides remain available without presenting unreviewed recordings as current product evidence.
