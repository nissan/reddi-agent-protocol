...
| Clean-checkout OSS smoke (#512) | `npm run check:oss-release-smoke` (repo root) | tests + no-spend examples + import smoke + pack inspection + RAP naming guard + RAP Assurance public-claim boundary gate + claim-boundary scan for the whole candidate set |
...
| RAP Assurance public-claim boundary gate | `npm run check:claims:public` (repo root) | owned prose, package manifests, and served caption tracks stay inside the boundary owned by [`PUBLIC-CLAIM-BOUNDARY.md`](./PUBLIC-CLAIM-BOUNDARY.md) (built-in negative control: `--negative-control` must exit 1) |
