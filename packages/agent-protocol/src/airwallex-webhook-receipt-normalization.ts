import {
  ...
  type AirwallexRailFixtureGuardrails,
  type AirwallexSupportState,
} from './airwallex-hosted-checkout-rail.js';
 ...
 */
export const AIRWALLEX_WEBHOOK_RECEIPT_SCHEMA_VERSION = 'reddi.airwallex-webhook-receipt.v1' as const;

/** Top-level draft flag: this schema signals draft/unverified external-rail shapes. */
export const AIRWALLEX_WEBHOOK_RECEIPT_DRAFT = true as const;

/** The issue that fixes this module's contract (probe-only cap, fixture-asserted HMAC). */
export const AIRWALLEX_WEBHOOK_RECEIPT_ISSUE = 580 as const;

/** The epic tracking the receipt-v1 revoked/contested-state gap this module documents. */
export const RECEIPT_V1_REVOCATION_GAP_ISSUE = 338 as const;
...
export type AirwallexWebhookEventFixture = {
  ...
  createdAt: string;
  data: { object: AirwallexWebhookEventObjectFixture };
};
...
export type AirwallexWebhookNormalizationReasonCode =
  | 'airwallex_webhook_probe_only_receipt'
  | 'webhook_fixture_malformed'
  | 'non_synthetic_fixture_rejected'
  | 'unknown_event_rejected'
  | 'revocation_event_not_receipt'
  | 'signature_missing_or_not_fixture_asserted'
  | 'live_signature_verification_rejected'
  | 'merchant_secret_material_rejected'
...
const REVOCABILITY_GAP_NOTE =
  'Card-rail receipts are revocable: a succeeded payment intent can later be refunded, disputed, or reversed. '
 ...
 */
export function normalizeAirwallexWebhookFixture(value: unknown): AirwallexWebhookNormalizationResult {
  ...
  const reasonCodes: AirwallexWebhookNormalizationReasonCode[] = [];
  const auditNotes: string[] = [];
  const push = (code: AirwallexWebhookNormalizationReasonCode, note: string) => {
  ...
  if (!isRecord(event) || !isNonEmptyString(event.name) || !isNonEmptyString(event.id) || !isRecord(event.data) || !isRecord((event.data as Record<string, unknown>).object)) {
  ...
  if ((AIRWALLEX_REVOCATION_EVENT_NAMES as readonly string[]).includes(eventName)) {
    push(
      'revocation_event_not_receipt',
      `Denied: '${eventName}' is a revocation-family event (refund/dispute/reversal/cancellation) and is explicitly NOT a receipt. `
  ...
  } else if (!(AIRWALLEX_NORMALIZABLE_EVENT_NAMES as readonly string[]).includes(eventName)) {
  ...
  if (!isNonEmptyString(object.currency) || !/^[A-Z]{3}$/.test(object.currency as string)) {
    push('webhook_fixture_malformed', 'Denied: object currency must be an ISO-4217-style 3-letter code (unverified — Airwallex docs).');
  }
  if (!isNonEmptyString(object.amountMinorUnits) || !/^\d{1,12}$/.test(object.amountMinorUnits as string)) {
  ...
  const fixture = value as unknown as AirwallexWebhookFixture;
  return {
    ok: true,
    receipt: {
      ...
      eventRef: {
        ...
        eventName: fixture.event.name as AirwallexNormalizableEventName,
...
function reject(
  reasonCodes: AirwallexWebhookNormalizationReasonCode[],
  auditNotes: string[],
): AirwallexWebhookNormalizationResult {
  return {
    ok: false,
...
function looksStructured(value: unknown): value is Record<string, unknown> & { synthetic: unknown; event: unknown; signature: unknown; bindingRefs: unknown } {
...
function hasCompleteBindingRefs(value: unknown): value is AirwallexWebhookFixtureBindingRefs {
  if (!isRecord(value)) return false;
  return (['evidenceRef', 'requestHash', 'responseHash', 'recipientRef', 'nonceRef', 'operatorApprovalRef'] as const)
...
const FIXTURE_SIGNATURE: AirwallexWebhookSignatureFixture = {
  header: 'x-signature',
  timestampHeader: 'x-timestamp',
...
const FIXTURE_BINDING_REFS: AirwallexWebhookFixtureBindingRefs = {
...
function makeFixture(event: AirwallexWebhookEventFixture): AirwallexWebhookFixture {
  return {
    schemaVersion: AIRWALLEX_WEBHOOK_RECEIPT_SCHEMA_VERSION,
...
function withEventOverride(base: AirwallexWebhookFixture, overrides: Partial<AirwallexWebhookEventFixture>): AirwallexWebhookFixture {
  const fixture = structuredClone(base);
  Object.assign(fixture.event, overrides);
  return fixture;
}

function withObjectOverride(base: AirwallexWebhookFixture, overrides: Record<string, unknown>): AirwallexWebhookFixture {
  const fixture = structuredClone(base);
...
const malformedEnvelope = (() => {
  const fixture = structuredClone(paymentIntentSucceeded) as unknown as Record<string, unknown>;
  fixture.event = { id: 'evt_fixture_awx_1005' };
  return fixture as unknown as AirwallexWebhookFixture;
})();
...
const nonFixtureAssertedSignature = (() => {
  const fixture = structuredClone(paymentIntentSucceeded) as unknown as { signature: Record<string, unknown> };
...
export function listAirwallexWebhookRejectionFixtures(): Array<{
  ...
  const expected: Record<keyof typeof airwallexWebhookRejectionFixtures, AirwallexWebhookNormalizationReasonCode> = {
  ...
  return (Object.keys(airwallexWebhookRejectionFixtures) as Array<keyof typeof airwallexWebhookRejectionFixtures>)
    .map((key) => ({
      key,
