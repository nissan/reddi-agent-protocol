...
export function validateBrowserWalletApprovalRecord(
  input: unknown,
  options: { now?: string | Date; allowFuturePartnerConfirmedAuddDevnet?: boolean } = {},
...
export function validateBrowserWalletIdentityCopyClaims(input: unknown): BrowserWalletIdentityCopyGuardResult {
  ...
  if (row.tokenProgram !== undefined && row.tokenProgram !== null) requireLiteral(row.tokenProgram, SPL_TOKEN_PROGRAM_ID, '$.tokenProgram', 'non_canonical_browser_wallet_identity', errors);
...
function validateProvider(value: unknown, path: string, errors: BrowserWalletApprovalValidationError[]): void {
  if (!isPlainObject(value)) {
...
function validatePrograms(value: unknown, path: string, errors: BrowserWalletApprovalValidationError[]): void {
  if (!isPlainObject(value)) {
...
function forbiddenCopyMatches(text: string, environment: ReddiPaymentEnvironmentLabel): Array<{ code: BrowserWalletApprovalValidationErrorCode; message: string }> {
