import {
  ...
  type Erc8004ExportOptions,
  type Erc8004ExportReasonCode,
  type Erc8004ExportSource,
} from './erc8004-export.js';
import {
 ...
 * export.
 */
export const ERC8004_CONFORMANCE_SCHEMA_VERSION = 'reddi.erc8004-export-conformance.v1' as const;
 ...
 */
export function verifyErc8004ExportAgainstSource(
  bundle: Erc8004ExportBundle,
  source: Erc8004RoundTripSource,
): Erc8004RoundTripResult {
  const checks: Erc8004RoundTripCheck[] = [];
  ...
  check(
    'spec_promoted_external_standard_honest',
    bundle.draft === false
      && bundle.externalStandard?.fieldShapesVerified === false
      && bundle.externalStandard?.deploymentClaim === false,
    'RAP-side spec is promoted (draft:false) while ERC-8004 field shapes stay marked unverified with no deployment claim',
  );
  check(
    'no_unknown_top_level_keys',
    Object.keys(bundle).every((key) => EXPECTED_TOP_LEVEL_KEYS.has(key)),
    'bundle must not invent fields outside the reddi.erc8004-export.v1 spec',
  );
  check(
    'guardrails_all_false',
    bundle.guardrails.minted === false
      ...
      && bundle.guardrails.trustImported === false,
    'no guardrail may imply an on-chain write, live verification, or trust import',
  );
  ...
  if (bundle.targetChainHint !== undefined) {
    check(
      'chain_hint_placeholder_unverified',
      CHAIN_HINT_PATTERN.test(bundle.targetChainHint)
        && bundle.crossReference.evmChainHint === bundle.targetChainHint
        && bundle.externalStandard.deploymentClaim === false,
      'targetChainHint must be a CAIP-2 eip155:* fixture placeholder with no deployment claim',
    );
  }
  ...
  if (bundle.identity === null) {
    ...
    check(
      'identity_protocol_matches',
      bundle.identity.registrationFile.protocol.name === receipt.protocol.name
        && bundle.identity.registrationFile.protocol.version === receipt.protocol.version,
      'registrationFile.protocol must equal receipt.protocol',
    );
    ...
    const expectedMetadata: Array<{ key: string; value: string }> = [
    ...
    check(
      'identity_metadata_derivable',
      JSON.stringify(bundle.identity.metadata) === JSON.stringify(expectedMetadata),
      'every identity metadata entry must recompute from the receipt with no invented keys',
    );
  }
  ...
  if (bundle.reputation !== null) {
    if (attestation === undefined) {
      ...
      const allowedTag2 = new Set(
        [attestation.rubric?.dimensions?.[0]?.id, receipt.job.type, 'unspecified'].filter((v): v is string => typeof v === 'string'),
      );
      ...
      check(
        'reputation_evidence_matches',
        bundle.reputation.payloadURI === attestation.evidenceRef && bundle.reputation.payloadHash === attestation.evidenceHash,
        'payloadURI/payloadHash must equal the attestation evidenceRef/evidenceHash (URI + hash only, never the raw payload)',
      );
      ...
      check(
        'reputation_source_is_final',
        receipt.attestationStatus === 'attested' && attestation.verdict === 'passed' && attestation.workStatus === 'completed' && attestation.receiptId === receipt.job.id,
        'an exported reputation entry must be backed by an attested receipt and a passed+completed attestation referencing it',
      );
    }
    ...
    if (bundle.reputation.credentialRef !== undefined) {
      ...
      if (verified.ok) {
        check(
          'credential_ref_matches_source',
          ref.credentialId === verified.credential.credential.id
            ...
            && ref.proof.publicKey === verified.credential.proof.publicKey,
          'credentialRef must reference the verified credential (id, subject, score, evidence hashes, public key) without duplicating its signature',
        );
        check(
          'credential_ref_binds_attestation_evidence',
          attestation !== undefined && ref.evidenceHashes.includes(attestation.evidenceHash),
          'the credential evidence hashes must include the attestation evidence hash backing the reputation entry',
        );
        check(
          'credential_ref_no_signature_duplicated',
          !JSON.stringify(ref).includes(verified.credential.proof.signature),
          'the portable credential signature must not be duplicated into the ERC-8004 payload',
        );
      }
    ...
    const exclusionCodes: Erc8004ExportReasonCode[] = [
    ...
    check(
      ...
      bundle.reasonCodes.some((code) => exclusionCodes.includes(code)),
      'when a supplied attestation does not export, the bundle must record the exclusion reason code',
    );
  }
  ...
  if (bundle.validation !== null) {
    if (attestation === undefined) {
      ...
      const expectedScore = Math.max(0, Math.min(100, Math.round(attestation.confidence)));
      check(
        'validation_matches_source',
        bundle.validation.requestHash === receipt.requestHash
          ...
          && bundle.validation.experimental === true,
        'validation payload must reconstruct from receipt request/response/evidence refs and the attestation score, and stay flagged experimental',
      );
      check(
        'validation_flagged_in_reason_codes',
        bundle.reasonCodes.includes('validation_registry_experimental'),
        'emitting a validation payload must be flagged with validation_registry_experimental',
      );
    }
  }
...
// ---------------------------------------------------------------------------
// Conformance fixtures — deterministic, offline, no chain refs beyond CAIP-2
// fixture placeholders marked unverified via the bundle externalStandard block.
// ---------------------------------------------------------------------------
...
function fixturePolicyDecision(): ReddiReceipt['policyDecision'] {
  return {
...
function fixtureReceipt(overrides: Partial<ReddiReceipt> = {}): ReddiReceipt {
  return {
...
function railNeutralGuardrails(): RailNeutralPaymentReceipt['guardrails'] {
  return {
    fixtureOnly: true,
 ...
 */
function probeOnlyRailNeutralReceiptFixture(): RailNeutralPaymentReceipt {
  return {
...
function bindingCandidateRailNeutralReceiptFixture(): RailNeutralPaymentReceipt {
  return {
 ...
 */
export function listErc8004ConformanceFixtures(): Erc8004ConformanceFixture[] {
  return [
    {
      kind: 'export',
 ...
 */
export function runErc8004ConformanceSuite(): Erc8004ConformanceSuiteResult {
  const cases: Erc8004ConformanceCaseResult[] = [];
  ...
  for (const fixture of listErc8004ConformanceFixtures()) {
    ...
    if (fixture.kind === 'eligibility') {
      ...
      for (const code of fixture.expected.reasonCodes) {
        if (!eligibility.reasonCodes.includes(code)) failures.push(`missing expected reason code ${code}`);
      }
    ...
    let roundTrip: Erc8004RoundTripResult | undefined;
