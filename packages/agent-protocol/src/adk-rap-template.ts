import {
  ...
  type FrameworkTemplateSupportState,
...
export const ADK_RAP_TEMPLATE_SCHEMA_VERSION = 'reddi.adk-rap-template.v1' as const;

export const ADK_RAP_TEMPLATE_FRAMEWORK: FrameworkTemplateId = 'adk';

export type AdkRapTemplateScenario =
...
export type AdkRapAgentSkillId =
  | 'rap.discover'
  | 'rap.quote'
...
export type AdkRapTemplateValidationReasonCode =
  | 'adk_rap_template_valid'
  | 'adk_template_malformed'
  | 'framework_contract_invalid'
  | 'framework_conformance_invalid'
  | 'missing_required_agent_skill'
  | 'missing_required_middleware'
  | 'missing_seller_wrapper_endpoint_helper'
  | 'missing_agent_card_rap_extension'
...
const REQUIRED_SKILLS: AdkRapAgentSkillId[] = [
  'rap.discover',
  'rap.quote',
  'rap.buyer-policy-preflight',
  'rap.operator-approval',
...
function requiredSkills(): AdkRapAgentSkill[] {
  return [
    {
      id: 'rap.discover',
      name: 'Discover',
 ...
 */
export function createAdkFrameworkTemplateContract(): FrameworkTemplateContract {
...
export function createAdkRapTemplateFixture(input: {
  ...
  if (!selectedContract.sellerProfile) throw new Error('adk_template_missing_seller_profile');
  const seller = selectedContract.sellerProfile;
  const agentCard: AdkA2aAgentCard = {
    schemaProfile: 'a2a-agent-card',
    framework: 'adk',
    protocolVersion: 'a2a/0.3.0-fixture',
    name: 'adk:rap-template:listing-writer',
    description:
      'Local no-live ADK A2A Agent Card carrying RAP discovery, quote, policy, and receipt metadata for a listing-writer agent.',
    preferredTransport: 'a2a-agent-card',
    capabilities: {
      streaming: false,
      pushNotifications: false,
  ...
  const fixture: AdkRapTemplateState = {
...
function isStructuredTemplate(value: unknown): value is AdkRapTemplateState {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return false;
  const candidate = value as Partial<AdkRapTemplateState>;
...
export function validateAdkRapTemplate(template: unknown): AdkRapTemplateValidationResult {
  ...
  const reasonCodes: AdkRapTemplateValidationReasonCode[] = [];
