...
export const QUASAR_REPUTATION_INTENT_COMPATIBILITY = {
    ...
    recordedDevnetDeployment: {
        ...
        usable: false,
        reason: 'Recorded devnet Quasar programs expect sha256(score||salt||job_id||program_id) with caller-supplied job_id/consumer_pk/specialist_pk and job_id-seeded PDAs. They predate the escrow-address job binding this contract describes.',
        remediation: 'Exercise Quasar only on a local Surfpool lane against locally built current-source programs (npm run test:surfpool:quasar-critical). No redeploy is claimed.',
    },
    offchainFieldNames: [
        'bindingId',
        'receiptId',
 ...
 */
export function evaluateQuasarReputationIntentSourceEligibility(source) {
 ...
 */
export function deriveQuasarReputationIntentPlan(input) {
    const binding = input.binding;
    const preview = input.preview;
    const reasonCodes = [];
    if (!isNonEmptyString(input.id))
        reasonCodes.push('missing_intent_id');
    ...
    const status = blocked ? 'blocked' : 'intent_ready';
    ...
    const normalizedReasonCodes = unique([
        ...(status === 'intent_ready' ? ['intent_ready'] : []),
        ...reasonCodes,
        'fixture_intent_only',
    ...
    const plan = {
        ...
        display: {
            ...
            explanation: status === 'intent_ready'
...
// Gates
// ---------------------------------------------------------------------------
function bindingGateReasonCodes(binding) {
    const reasonCodes = [];
...
function compatibilityGateReasonCodes(compatibility, binding) {
    const reasonCodes = [];
    ...
    if (!compatibilityGuardrailsAreSafe(compatibility))
        reasonCodes.push('unsafe_live_guardrail');
    if (compatibility.registrationStatus === 'blocked')
        reasonCodes.push('quasar_compatibility_blocked');
    if (!compatibilitySubjectMatchesBinding(compatibility, binding))
        reasonCodes.push('compatibility_subject_mismatch');
    return reasonCodes;
}
function previewGateReasonCodes(preview, binding) {
    ...
    if (!previewGuardrailsAreSafe(preview))
        reasonCodes.push('unsafe_live_guardrail');
    if (preview.status !== 'preview_ready')
        reasonCodes.push('preview_not_ready');
...
function laneStatesFor(binding, gateReasonCodes) {
    ...
    const ratingLaneReason = binding?.reputationEventDraft
        ? 'malformed_source_metadata'
...
function intentRecordFor(kind, binding, preview, compatibility) {
...
function attestationProgramLane(instructionName, discriminator) {
    return {
        lane: 'quasar-attestation',
        deploymentsRef: QUASAR_REPUTATION_INTENT_COMPATIBILITY.deploymentsRef,
        instructionName,
 ...
 */
function ratingScoreFor(binding) {
    ...
    return Math.min(QUASAR_REPUTATION_INTENT_COMPATIBILITY.scoreRange.max, Math.max(QUASAR_REPUTATION_INTENT_COMPATIBILITY.scoreRange.min, Math.round(rubricScore / 10)));
}
// ---------------------------------------------------------------------------
// Shape helpers
// ---------------------------------------------------------------------------
...
function bindingEvidenceIsComplete(binding) {
...
function compatibilityGuardrailsAreSafe(compatibility) {
...
function compatibilitySubjectMatchesBinding(compatibility, binding) {
    if (!binding?.source)
        return false;
    if (isNonEmptyString(binding.source.listingId))
        return compatibility.listingId === binding.source.listingId;
    if (isNonEmptyString(binding.source.profileId))
        return compatibility.profileId === binding.source.profileId;
    return false;
}
...
function evidenceSummaryFor(input, binding, preview) {
    return {
...
function isBlockingReasonCode(code) {
