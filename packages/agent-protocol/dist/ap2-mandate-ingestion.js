...
import { validateBuyerAuthorityPolicy, } from './buyer-authority-policy.js';
 ...
 */
export const AP2_MANDATE_INGESTION_DRAFT = false;
/**
...
const AP2_MANDATE_TYPE_VOCABULARY = [
    'intent', 'cart', 'payment',
 ...
 */
export const AP2_MANDATE_FIELD_PROVENANCE = [
 ...
 */
export const AP2_UNSUPPORTED_FIELDS = [
...
export const AP2_AUTHORIZATION_NETWORK = 'ap2-authorization-fixture';
...
export function ingestAp2Mandate(mandate, now) {
    ...
    if (!isStructuredMandate(mandate)) {
        ...
        const rawType = String(mandate.mandateType);
 ...
 */
export function composeAp2MandateWithLocalPolicy(localPolicy, ingestion) {
    const reasonCodes = [];
    const auditNotes = [];
    const blocked = () => ({
        schemaVersion: AP2_MANDATE_INGESTION_SCHEMA_VERSION,
    ...
    const localCap = local.spendCaps.find((cap) => (cap.asset === currency && cap.network === AP2_AUTHORIZATION_NETWORK && cap.window === 'per_request'));
    ...
    if (comparison !== null && comparison < 0) {
        composedCapUnits = mandateCapRaw;
    ...
    if (!Number.isNaN(localExpiryMs) && !Number.isNaN(mandateExpiryMs) && mandateExpiryMs < localExpiryMs) {
        composedExpiresAt = derived.expiresAt;
    ...
    let composedApproval = structuredClone(local.operatorApproval);
...
function isBindableRefShape(ref) {
    if (!isRecord(ref))
        return false;
    if (!Object.keys(ref).every((key) => MANDATE_REF_ALLOWED_KEYS.has(key)))
        return false;
    if (refCarriesSignatureMaterial(ref))
        return false;
    const candidate = ref;
...
function refCarriesSignatureMaterial(ref) {
    if (!isRecord(ref))
        return false;
    for (const [key, value] of Object.entries(ref)) {
        if (SIGNATURE_KEY_PATTERN.test(key))
            return true;
        if (typeof value === 'string' && (SIGNATURE_VALUE_PATTERN.test(value) || SENSITIVE_VALUE_PATTERN.test(value)))
            return true;
    }
    return false;
}
function deriveConstraints(mandate, currency, maxAmountUnits) {
...
function hasVdcEnvelope(value) {
    if (!isRecord(value))
        return false;
    if (typeof value.mandateType !== 'string')
        return false;
    const vdc = value.vdc;
    return isRecord(vdc)
...
function isStructuredMandate(value) {
    return hasVdcEnvelope(value)
        && AP2_MANDATE_TYPE_VOCABULARY.includes(value.mandateType);
}
 ...
 */
function containsSignatureMaterial(value) {
    if (typeof value === 'string')
        return SIGNATURE_VALUE_PATTERN.test(value);
    if (!value || typeof value !== 'object')
        return false;
    if (Array.isArray(value))
        return value.some(containsSignatureMaterial);
    return Object.entries(value).some(([key, nested]) => (SIGNATURE_KEY_PATTERN.test(key) || containsSignatureMaterial(nested)));
}
 ...
 */
export function ap2FixtureCentiUnits(value) {
...
function dedupeComposition(codes) {
...
export const AP2_RAIL_SUPPORT_STATE_MATRIX = [
    {
        ...
        description: 'Finalized Checkout/Cart + Payment mandate present; VDC signature treated as fixture-asserted (opaque, not verified live); buyer-authority constraints derivable and composable with a local policy that always wins; no settlement claim.',
        draft: true,
        confidence: 'low',
    ...
    },
    {
        ...
        description: 'Only an Open/Intent mandate (constraints, no finalized cart) — usable for preflight/planning and parser tests, not for authorizing a specific spend. Probe-only refs never compose with a policy and never bind to a receipt.',
        draft: true,
        confidence: 'low',
    ...
    },
    {
        ...
        mandateTypes: ['cart', 'payment', 'checkout_closed', 'payment_closed'],
        description: 'Any mandate asserting completed settlement, custody, or finality — or carrying credential/PAN/signature material outside the opaque VDC slot — is rejected. A future live lane must first define live VDC signature verification, wallet custody, spend limits, replay handling, operator approval, and rollback.',
        draft: true,
        confidence: 'low',
