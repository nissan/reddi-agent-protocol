 ...
 */
import { normalizeAiCatalogProviderTrustRecord, providerTrustFixtures, } from './provider-trust.js';
export const SOURCE_TRUST_CONFORMANCE_MATRIX_SCHEMA_VERSION = 'reddi.source-trust-conformance-matrix.v1';
/** Where the boundary between discovery relevance and trust/policy/payment/evidence/reputation is defined. */
export const SOURCE_TRUST_BOUNDARIES_DOC_REF = 'docs/DISCOVER-DECIDE-PROVE-BOUNDARIES.md';
export const SOURCE_TRUST_STATES = [
    'trusted',
    'listed_untrusted',
    'claimed',
    'unverified',
...
export const SOURCE_TRUST_REQUIRED_CASES = [
    'malformed_metadata',
    'credential_leakage',
...
export const SUPPORTED_AUTH_MD_CREDENTIAL_TYPES = ['access_token', 'api_key'];
/** Supported auth.md registration types (aligned with the auth.md discovery lane). */
export const SUPPORTED_AUTH_MD_REGISTRATION_TYPES = ['agent-provider', 'email-verification', 'anonymous'];
/**
 ...
 * exact allowlist.
 */
export const SUPPORTED_AUTH_MD_IDENTITY_ASSERTIONS = ['publisher_domain', 'oidc_id_token'];
const STATE_LABELS = {
    trusted: 'trusted',
...
function readAuthSurface(source) {
 ...
 */
function evaluateAuthSurface(surface, basePath, findings) {
    for (let index = 0; index < surface.scopes.length; index += 1) {
        ...
        if (offeredTo.includes('anonymous')) {
            findings.push(finding('anonymous_write_scope_rejected', 'blocked', `${basePath}.scopes[${index}]`, `write scope "${scope.name}" is offered to anonymous registrations; anonymous write scopes fail closed.`));
        }
    }
    for (const credentialType of surface.credentialTypes) {
        if (!SUPPORTED_AUTH_MD_CREDENTIAL_TYPES.includes(credentialType)) {
            findings.push(finding('unsupported_credential_type', 'blocked', `${basePath}.credentialTypes`, `credential type "${credentialType}" is not supported (supported: ${SUPPORTED_AUTH_MD_CREDENTIAL_TYPES.join(', ')}).`));
        }
    }
    for (const assertion of surface.identityAssertions) {
        if (!SUPPORTED_AUTH_MD_IDENTITY_ASSERTIONS.includes(assertion)) {
            findings.push(finding('unsupported_identity_assertion', 'blocked', `${basePath}.identityAssertions`, `identity assertion "${assertion}" is not supported (supported: ${SUPPORTED_AUTH_MD_IDENTITY_ASSERTIONS.join(', ')}).`));
        }
    }
}
function hasAuthMdTrustClaims(trust) {
    if (!trust)
        return false;
    return Boolean(trust.trustManifest
...
function runAuthMdGates(metadata, rapVerification) {
    const findings = [];
    ...
    if (trust && typeof trust.status === 'string' && trust.status.toLowerCase() === 'verified') {
        findings.push(finding('self_asserted_verification_ignored', 'warning', '$.trust.status', 'external metadata self-asserts a verified status; external claims cannot verify themselves — routed to human review.'));
    }
...
function appendVerificationFindings(hasTrustClaims, rapVerification, findings) {
...
function deriveState(outcome, rapVerification) {
    ...
    if (blockedFindings.some((item) => item.code !== 'rap_verification_failed'))
        return 'blocked';
...
function registryStatusFor(state, hasTrustClaims) {
    switch (state) {
        case 'trusted':
            return 'verified';
        case 'failed_verification':
...
export function classifySourceTrustCandidate(input) {
    const gatesRun = input.gates !== 'not_run';
    const findings = [];
    let outcome = { findings: [], hasTrustClaims: false };
    findings.push(finding('listed_untrusted_on_entry', 'info', '$', `${input.source.kind} metadata enters RAP as untrusted source metadata; no trust is granted by ingestion.`));
    if (gatesRun) {
    ...
    if (input.relevance?.score !== undefined) {
        findings.push(finding('relevance_ignored_for_trust', 'info', '$.relevance.score', `relevance score ${input.relevance.score} is recorded for search/ranking only and is ignored by every trust gate.`));
    }
    const state = gatesRun ? deriveState(outcome, input.rapVerification) : 'listed_untrusted';
    return {
...
const AUTH_MD_TRUSTED_METADATA = {
 ...
 */
export const sourceTrustConformanceFixtureCases = {
    trustedAuthMd: {
...
export function buildSourceTrustConformanceMatrix(cases = sourceTrustConformanceFixtureCases) {
    ...
    const states = Object.fromEntries(SOURCE_TRUST_STATES.map((state) => [state, 0]));
    const requiredCases = Object.fromEntries(SOURCE_TRUST_REQUIRED_CASES.map((item) => [item, 0]));
    const sourceKinds = { 'auth-md': 0, 'ai-catalog': 0 };
    for (const row of rows) {
        states[row.state] += 1;
        sourceKinds[row.sourceKind] += 1;
        if (row.requiredCase)
            requiredCases[row.requiredCase] += 1;
    }
