...
export function createReddiReceipt(input) {
...
export function validateReddiReceipt(input) {
    const errors = [];
...
function validatePolicyDecision(value, errors) {
    ...
    if (typeof decision.allowed !== 'boolean')
        errors.push(error('malformed_receipt', '$.policyDecision.allowed', 'allowed must be boolean'));
...
function requireLiteral(value, expected, path, errors) {
    if (value !== expected)
        errors.push(error('malformed_receipt', path, `expected ${expected}`));
}
function requireString(value, path, errors) {
    if (!isNonEmptyString(value))
        errors.push(error('malformed_receipt', path, 'required string is missing'));
}
function requirePositiveIntegerString(value, path, errors) {
