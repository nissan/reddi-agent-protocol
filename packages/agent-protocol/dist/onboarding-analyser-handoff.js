 ...
 * and reviewed).
 */
export const ONBOARDING_INTAKE_DESCRIPTOR_SCHEMA_VERSION = 'reddi.onboarding-intake-descriptor.v1';
export const ONBOARDING_CAPABILITY_INVENTORY_SCHEMA_VERSION = 'reddi.onboarding-capability-inventory.v1';
export const ONBOARDING_RAP_PROFILE_DRAFT_SCHEMA_VERSION = 'reddi.onboarding-rap-profile-draft.v1';
export const ONBOARDING_READINESS_RESULT_SCHEMA_VERSION = 'reddi.onboarding-readiness-result.v1';
export const ONBOARDING_SELLER_WRAPPER_DRAFT_SCHEMA_VERSION = 'reddi.onboarding-seller-wrapper-draft.v1';
export const ONBOARDING_LISTING_EXPORT_DRAFT_SCHEMA_VERSION = 'reddi.onboarding-listing-export-draft.v1';
export const ONBOARDING_ANALYSER_HANDOFF_SCHEMA_VERSION = 'reddi.onboarding-analyser-handoff.v1';
export const ONBOARDING_ANALYSER_FIXTURE_MATRIX_SCHEMA_VERSION = 'reddi.onboarding-analyser-fixture-matrix.v1';
/** Issue anchors so downstream implementations can trace contract ownership. */
export const ONBOARDING_ANALYSER_HANDOFF_ISSUE = 509;
...
export const ONBOARDING_FIXTURE_CLASSES = Object.freeze([
    'mcp-metadata',
    'openapi',
    'a2a-card',
    'ard-ai-catalog',
...
const FORBIDDEN_OPERATION_REASONS = {
 ...
 */
export function verifyOnboardingField(field, evidenceRefs) {
...
export const ONBOARDING_READINESS_LANES = Object.freeze([
    'capability_fit',
    'source_identity',
...
const SIDE_EFFECT_RISKS = ['none', 'read', 'write', 'execute'];
const SOURCE_KINDS = [
    'mcp-metadata',
    'openapi',
    'a2a-card',
...
function intakeError(code, path, message) {
...
function isPrivateOrLocalUrl(parsed) {
...
function validateDeclaredCapability(value, path, errors) {
    ...
    for (const key of ['description', 'inputShape', 'outputShape']) {
    ...
    if (value.sideEffectHint !== undefined && !SIDE_EFFECT_RISKS.includes(value.sideEffectHint)) {
 ...
 */
export function validateOnboardingIntakeDescriptor(input) {
    ...
    if (!isPlainObject(input.declaredMetadata)) {
        ...
        for (const key of ['authHints', 'pricingHints', 'rateLimitHints', 'trustClaims']) {
        ...
        if (trustClaims.length > 0) {
            errors.push(intakeError('untrusted_imported_content_rejected', '$.declaredMetadata.trustClaims', 'imported content cannot self-assert trust, verification, approval, or live payment status'));
        }
    }
    if (input.operatorProvided !== undefined) {
        if (!isPlainObject(input.operatorProvided)) {
            ...
            for (const key of ['displayName', 'description', 'contactRef']) {
    ...
    if (input.requestedOperations !== undefined) {
        if (!isOptionalStringArray(input.requestedOperations)) {
            ...
            input.requestedOperations.forEach((operation, index) => {
                const reason = FORBIDDEN_OPERATION_REASONS[operation];
                if (reason) {
    ...
    return errors.length === 0
        ? { ok: true, descriptor: input }
        : { ok: false, errors };
}
// ---------------------------------------------------------------------------
// Stage builders (pure, deterministic)
// ---------------------------------------------------------------------------
function discoveredField(value) {
...
function userProvidedField(value) {
    if (value === undefined) {
        return { provenance: 'blocked', evidenceRefs: [], blockedReason: 'missing_evidence' };
    }
    return { provenance: 'user_provided', value, evidenceRefs: [] };
}
function inferSideEffectRisk(capability) {
    const haystack = [capability.name, capability.description ?? ''].join(' ').toLowerCase();
    if (/\b(exec|execute|shell|command|deploy|install|script)\b/.test(haystack))
        return 'execute';
    if (/\b(write|create|update|delete|mutate|post|put|patch)\b/.test(haystack))
        return 'write';
    if (/\b(read|get|list|search|fetch|query|lookup)\b/.test(haystack))
        return 'read';
    return 'none';
}
function rawSnapshotRefs(source) {
    return [
...
export function buildOnboardingCapabilityInventory(descriptor) {
    const diagnostics = [];
    ...
    const entries = descriptor.declaredMetadata.capabilities.map((capability, index) => {
        const sideEffectRisk = capability.sideEffectHint !== undefined
...
const REQUIRED_PAYMENT_FIELDS = ['settlementAddress', 'network', 'price', 'currency'];
function missingPaymentFields(metadata) {
    if (!metadata)
        return [...REQUIRED_PAYMENT_FIELDS];
...
export function buildOnboardingRapProfileDraft(descriptor, inventory) {
    const endpointUrls = descriptor.declaredMetadata.endpointUrls ?? [];
    ...
    const displayName = operatorDisplayName !== undefined
...
function lane(key, status, reasonCodes, recommendations) {
 ...
 */
export function analyzeOnboardingReadiness(profile) {
    const lanes = [
...
export function buildOnboardingSellerWrapperDraft(profile) {
...
export function buildOnboardingListingExportDraft(profile, readiness) {
    const states = [
        {
            code: 'listing_draft',
            severity: 'info',
 ...
 */
export function runOnboardingAnalyserHandoff(input) {
 ...
 */
export function partitionOnboardingEntryFieldsByProvenance(entry) {
    const partition = {
        discovered: [],
        inferred: [],
        user_provided: [],
...
function fixtureDescriptor(intakeId, sourceKind, overrides = {}) {
 ...
 */
export function listOnboardingAnalyserFixtureMatrix() {
    return [
        {
