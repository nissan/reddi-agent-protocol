import { AP2_AUTHORIZATION_NETWORK, AP2_MANDATE_INGESTION_FIXTURE_NOW, AP2_MANDATE_INGESTION_SCHEMA_VERSION, ap2FixtureCentiUnits, ap2MandateFixtures, bindMandateToReceipt, composeAp2MandateWithLocalPolicy, hashAp2Mandate, ingestAp2Mandate, } from './ap2-mandate-ingestion.js';
import { evaluateBuyerAuthorityPolicy, } from './buyer-authority-policy.js';
 ...
 */
export const AP2_MANDATE_CONFORMANCE_SCHEMA_VERSION = 'reddi.ap2-mandate-conformance.v1';
 ...
 */
export function verifyAp2IngestionAgainstMandate(result, mandate) {
    const checks = [];
    const check = (id, ok, detail) => {
        checks.push({ id, ok, detail });
    };
    check('ingestion_authorizing', result?.supportState === 'ap2_mandate_fixture' && result?.derived !== null, 'a non-authorizing result (probe-only / rejected) has no source round-trip; fail-closed lanes are asserted via reason codes');
    ...
    check('promoted_external_standard_honest', result.draft === false
        && result.externalStandard?.fieldShapesVerified === false
        && result.externalStandard?.signatureVerification === 'fixture-asserted'
        && result.externalStandard?.settlementRail === false, 'RAP-side contract is promoted (draft:false) while AP2 field shapes stay marked unverified, signatures fixture-asserted, and no settlement rail is claimed');
    check('no_unknown_top_level_keys', Object.keys(result).every((key) => EXPECTED_RESULT_TOP_LEVEL_KEYS.has(key)), 'result must not invent fields outside the reddi.ap2-mandate-ingestion.v1 contract');
    check('guardrails_all_safe', result.guardrails.fixtureOnly === true
        ...
        && result.guardrails.settlementFinalityClaim === false, 'no guardrail may imply live verification, key handling, custody, payment, or settlement');
    ...
    check('ref_consistent_and_claimless', result.mandateRef.mandateType === mandate.mandateType
        ...
        && result.mandateRef.settlementFinalityClaimed === false, 'the mandate ref must mirror the mandate type / support state, stay fixture-asserted, and claim no settlement finality');
    // Derived constraints recompute from the mandate source.
    const derived = result.derived;
    const expectedCurrency = AP2_CURRENCY_TO_RAP_ASSET[String(mandate.payment?.currency ?? mandate.cart?.total.currency ?? '').trim().toUpperCase()];
    check('derived_currency_recomputes', derived.allowedCurrencies.length === 1 && derived.allowedCurrencies[0] === expectedCurrency, `allowedCurrencies must recompute to [${expectedCurrency}] from the mandate payment/cart currency`);
    const expectedCap = ap2FixtureCentiUnits(mandate.payment?.budgetCap ?? mandate.payment?.amount ?? mandate.cart?.total.amount ?? '0');
    check('derived_cap_recomputes', derived.spendCaps.length === 1
        && expectedCap !== null
        ...
        && derived.spendCaps[0].window === 'per_request', 'the derived spend cap must recompute from budgetCap → amount → cart total, per_request, on the ap2-authorization-fixture lane');
    ...
    check('derived_merchant_refs_recompute', JSON.stringify(derived.sellerAllowlistAdditions.sellerIds) === JSON.stringify(expectedSellerIds)
        && JSON.stringify(derived.sellerAllowlistAdditions.endpointIds) === JSON.stringify(expectedEndpointIds), 'seller refs must recompute as namespaced ap2-merchant refs from the mandate merchant id');
    check('derived_expiry_recomputes', derived.expiresAt === mandate.expiresAt, 'derived expiry must equal the mandate expiry');
    check('derived_operator_approval_recomputes', derived.operatorApprovalRequired === (mandate.humanPresent === false), 'operator approval must be required exactly when the mandate was human-not-present');
...
// ---------------------------------------------------------------------------
export function ap2LocalBuyerAuthorityPolicyFixture(overrides = {}) {
    return {
...
function authorizedRef(mandateKey) {
    return ingestAp2Mandate(structuredClone(ap2MandateFixtures[mandateKey]), NOW).mandateRef;
}
/**
 ...
 */
export function listAp2ConformanceFixtures() {
    return [
        {
            kind: 'ingestion',
        ...
        },
        {
            ...
            tamper: (mandate) => {
                if (mandate.cart)
                    mandate.cart.total.amount = '26.00';
            },
 ...
 */
export function runAp2MandateConformanceSuite() {
    const cases = [];
    for (const fixture of listAp2ConformanceFixtures()) {
        const failures = [];
        let roundTrip;
        if (fixture.kind === 'ingestion') {
            const mandate = structuredClone(ap2MandateFixtures[fixture.mandateKey]);
            ...
            for (const code of fixture.expected.reasonCodes) {
                if (!result.reasonCodes.includes(code))
        ...
        if (fixture.kind === 'composition') {
            ...
            for (const code of fixture.expected.reasonCodes) {
                if (!composition.reasonCodes.includes(code))
                    failures.push(`missing expected reason code ${code}`);
            }
        ...
        for (const code of fixture.expected.reasonCodes) {
            if (!binding.reasonCodes.includes(code))
                failures.push(`missing expected reason code ${code}`);
        }
        ...
        if (fixture.expected.ok && binding.receipt !== null) {
            ...
            if (!validation.ok)
                failures.push('the bound receipt must still pass reddi.receipt.v1 validation');
            const bindingMeta = bound.metadata?.ap2MandateRef;
