...
- Browser-wallet safety preflight contracts and offline validation helpers (`@reddi/agent-protocol/browser-wallet-approval`): single-use Devnet approval schema (`reddi.browser-wallet.single-use-approval.v1`) with ordered timestamps, the canonical Devnet USDC mint pinned for the gated USDC lane, official Solana mainnet mints rejected on every asset path, and partner-confirmed AUDD Devnet mint/rail binding; dormant Tier 1 local browser harness contract; and claim-local copy guard rejecting official AUDD, grant-eligible, observed settlement, and controlled-live claims across safety rows.
