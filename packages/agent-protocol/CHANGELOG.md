...
- Intent-draft labels must now match the plan's derived rail in both directions: labels that understate a live rail and labels that overstate a fixture/devnet rail both throw `audd_payment_plan_label_environment_mismatch`. Receipt/evidence binding likewise rejects a payment observation whose environment label is more live than the rail its observed mint/network belongs to.
