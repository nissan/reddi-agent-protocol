...
- `NEXT_PUBLIC_DEMO_PROGRAM_TARGET=quasar` selects the Quasar target: refused on `mainnet`; refused on `local-surfpool` unless four distinct valid local program ids are supplied; on `devnet` it resolves for disclosure only, and every exported Quasar instruction builder calls `assertQuasarProgramTargetUsable()`, which refuses it because the recorded deployment is not submission-ready.
