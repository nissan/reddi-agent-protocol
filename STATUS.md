...
Delivered:
- **`reddi.ranking-explainability.v1`** (`packages/agent-protocol/src/ranking-explainability.ts`, new subpath export `@reddi/agent-protocol/ranking-explainability`): per-candidate explainability block — source identity, capability match (`relevance_only_not_trust` + matched fields), trust state, payment policy fit, health/freshness (never probed), all-eight-gates array (trust/policy/quote/evidence/payment/budget/**settlement/attestation** — structurally always present, `not_evaluated` never counts as passed), reason-coded rejection reasons, selected/rejected/deferred selection state, embedded `reddi.source-diagnostics.v1` block, literal-false guardrails. `explainSourceRanking()` orders by relevance only and marks (never reorders/hides) gate-failing candidates.
- **`reddi.ard-candidate-diagnostics.v1`** (`lib/manager/ard-candidate-diagnostics.ts`): composes the #577 hosted actionability matrix, the #593 row state/`diagnosticsProjection`, the #606 `listingProjection` (reputation state), and the explainability block for hosted search result items — with relevance, publisher identity, trust evidence, policy decision, budget/payment fit, receipt/evidence history, and reputation state as separate sections.
- **`reddi.supervisor-run-diagnostics.v1`** (`lib/manager/supervisor-run-diagnostics.ts`): read model over the existing economic-demo fixtures (no live runs) — run linkage (run id, supervisor, mode, receipt refs), per-child invocation state (planned/paid/attested/blocked) with failure reasons for blocked children, settlement/audit state (payment proof statuses, attestation results, reputation events), and reason-coded `constraintViolations` for settled-without-attestation and blocked-with-settlement children.
- Tests: `packages/agent-protocol/tests/ranking-explainability.test.ts` (7 tests incl. the 0.99-relevance-still-rejected pattern per gate and the all-source-kinds settlement/attestation structural test), `lib/__tests__/manager-ard-candidate-diagnostics.test.ts` (5), `lib/__tests__/manager-supervisor-run-diagnostics.test.ts` (7, incl. tamper-detection proofs that the bypass audit is non-vacuous).
- Docs/BDD: `bucket-b-discovery.feature` scenarios `@B3.1`–`@B3.5` + FEATURE-INDEX verify commands + BDD-SCENARIO-ID-MAP rows; `docs/conformance/source-trust-matrix/README.md` explainability-requirement section; boundaries doc §4 Discover updated with the landed module refs.

Validation: package suite 556/556 (node --test), full jest lib suite 120 suites / 520 tests green (incl. OpenClaw/Hermes/pi source-adapter profile tests), `npm run test:source:conformance` (openclaw smoke incl. `pnpm build`) all steps green, `check:rap:naming`, `test:bdd:index`, package `check:exports` (97 targets), `check:package:artifacts` all OK. `tsc --noEmit` error count unchanged vs main (40 pre-existing lines, none in new files).
## Latest Update — ARD/open-agent candidate detail UI (#382) (2026-07-17 AEST)

Implemented GitHub issue #382 (parent UX epic #380; deps #365/#577/#593 landed, built directly on #381/PR #605) in isolated worktree `projects/reddi-agent-protocol-worktree-382-candidate-detail` on branch `feat/382-ard-candidate-detail`, branched from `origin/main` at `9d4a73d8` (the #605 merge). Scope: `/agents` candidate detail route, discovery components, `lib/discovery`, tests, evidence — onboarding surfaces (concurrent #385) and `packages/agent-protocol` + `lib/economic-demo` + `lib/manager` marketplace-listings/offchain-reputation-preview (concurrent #394) deliberately untouched.

**Dependency audit (#376):** the #380 epic map listed #382 as "blocked on #376", but #376 is the *generation* side (emit ARD AI Catalog fragments + RAP registry/listing payloads for onboarded agents). A read-only DETAIL view needs none of that: every acceptance field (publisher/host, resource identifier, media type, endpoint/url, capabilities, auth/payment metadata, trust manifest, validation state, raw snapshot reference, lifecycle-state separation) is already present in — or honestly absent from — the shipped read models (#369 hosted catalog search, #383 ARD static-stack review fixtures, Circle x402 / Pay.sh fixture catalogs, #577 matrix adapters). Where a field doesn't exist yet (endpoint URLs and trust-manifest refs for hosted listings, health checks/pricing detail that #376's generated payloads will add), the detail renders a literal "unavailable" — which the issue itself requires. So #382 was implementable in full; no #376 slice was deferred. When #376 lands, its richer listing payloads can flow through the same sections without schema change.

Delivered:
- **Detail read model** `lib/discovery/candidate-detail.ts` (`reddi.discovery-candidate-detail.v1`), served by new read-only `GET /api/discovery/candidates/[id]` (200/400/404 by availability): parses the #381 card ids (`hosted-rap:…`, `hosted-rap:blocked:…`, `ard-catalog:…`, `circle-x402:…`, `pay-sh:…`), embeds the **exact #381 card model built by the same builders** (card/detail can never disagree), derives the **full six-lane #577 matrix** via `deriveHostedDiscoveryActionabilityMatrix` / `deriveDiscoveryActionabilityMatrix` where an adapter exists, and sets an honest `matrixUnavailableReason` where none does (blocked hosted export records; Circle x402/Pay.sh externally listed snapshots — inventing lane states would be new trust logic). Five-stage lifecycle strip separates `discovered`/`RAP-wrapped`/`attested`/`payment-ready`/`hireable` with per-source notes; every candidate served is discovery-stage only so later stages are structurally never "reached". Sections (provenance/identity/endpoint/payment/trust-manifest) use `value: null` → "unavailable" rendering; plus capability groups/tags, full untruncated gating reasons, validation findings (from #383 review items / adapter diagnostics / hosted export blocks), recovery actions (from the fixtures' `recommendedAction` vocabulary), evidence + raw snapshot refs, source guardrail notes, all-false guardrails, boundary copy. Fail-closed availabilities: `not_found`, `source_unavailable` (non-ingested Circle x402/Pay.sh artifacts), `unsupported_id` (malformed ids and registry-native facets — RAP-native specialists keep the existing `/agents/[wallet]` detail, and the panel says so).
- **UI**: deep-linkable `app/agents/candidates/[id]/page.tsx` (Suspense-wrapped `useSearchParams` client tree, same pattern as `/agents`) + `components/discovery/CandidateDetail.tsx`; `?source=…&task=…` composes with the #381 facet state — cards carry the current filter state into the detail link and the back link restores it. `MarketplaceCandidateCard` gains the single "View details" internal link; the #605 Playwright zero-links invariant was tightened rather than dropped (cards may contain only internal `/agents/candidates/…` anchors, still no other buttons/verbs). Detail view itself renders zero buttons; endpoint/service URLs are text metadata, never links; unknown ids render honest panels with recovery actions, never broken pages.
- **Tests**: 18 new jest tests (`lib/__tests__/discovery-candidate-detail.test.ts`): id-parse fail-closed cases, hosted untrusted/blocked, ARD valid/malformed (matrix lane order locked to `DISCOVERY_ACTIONABILITY_LANE_ORDER`, card `toEqual` drift-lock, honest-null fields), Circle x402/Pay.sh via injected catalogs + repo-default `source_unavailable`, registry-native `unsupported_id`, determinism, lifecycle/guardrail invariants on every result, live-import mocks throwing (registry bridge, @solana/web3.js). 8 new Playwright tests (`e2e/agents-candidate-detail.spec.ts`): card→detail navigation with facet state, six-lane matrix + lifecycle rendering, honest unavailable fields, both blocked shapes, back-link composition, fail-closed unknown/unsupported/unavailable pages, keyboard/focus operability, no-live-affordance scan. Evidence spec `e2e/agents-candidate-detail-evidence-382.spec.ts`.
- **Evidence** (`docs/evidence/382/`): 24 PNGs at 375/768/1280 (ARD-imported detail, hosted untrusted detail, ARD blocked, hosted blocked-export, not-found, source-unavailable, RAP-native `/agents/[wallet]` for comparison) + 3 keyboard/focus proof shots + `keyboard-focus-flow.webm` (video not trace, per the #506 chrome-channel precedent) + README. Captured to `test-results/evidence-382/`, copied after the LAST Playwright run per the #498/#499 pitfalls.

Validation: focused jest 18/18; full `npx jest` 119 suites / 526 tests green; Playwright (dedicated dev server on :3382 via `PLAYWRIGHT_BASE_URL` — :3010 held by concurrent agents) `agents-candidate-detail.spec.ts` 8/8; regression `agents-source-facets.spec.ts` + `agents.spec.ts` + `navigation.spec.ts` 15/15 (one contention flake on the composable-filter test in the first batch, passes solo and on clean re-run); evidence spec 4/4 (last run); `npx eslint` clean on touched files; `npx tsc --noEmit` no errors in touched files (repo baseline unchanged at 40 lines); `npm run check:rap:naming` PASS; `npm run build` PASS (`/agents/candidates/[id]` + `/api/discovery/candidates/[id]` dynamic); `git diff --check` clean. Dev-only Suspense hydration duplicate handled with the #605 settle-polling pattern in both specs.

Boundary note: read-only end to end — the detail route/API perform no network call, paid call, endpoint invocation, wallet/RPC action, publication, or trust/reputation mutation; Playwright enforces zero buttons and internal-only anchors on the detail view.

RESUME FROM HERE: #376 can later enrich the same detail sections with generated listing-payload fields (health checks, pricing, trust-manifest refs); the discovery review workspace and marketplace listing preview can deep-link `/agents/candidates/[id]` directly.
...
**Dependency-audit verdict (mandatory pre-check)**: #385's listed deps #372/#373/#374 are all still open, but their substance for this editor is already on `main` — PR #575 (`onboarding-analyser-handoff.ts`, `reddi.onboarding-*.v1`) carries the generated RAP profile draft (`reddi.onboarding-rap-profile-draft.v1`) with five-way field provenance (discovered/inferred/user_provided/verified/blocked) and the readiness result across all 11 lanes (#372/#373 substance), and PR #576 (`onboarding-state-machine.ts`) carries the 12 assistant states with blocking-gate semantics (#374 substance). Built directly on the merged contracts; did NOT wait on or re-implement #372/#373/#374. Genuine contract gaps (named honestly in the UI, not fabricated): tools/skills as distinct-from-capabilities fields, downstream calls, and context requirements are not carried by the profile-draft contract — rendered as explicit "unavailable" sections; `verified` provenance is structurally unreachable at generation time (requires evidence refs) and renders as `verified × 0` with explanation.

Delivered:
- **Review/editor route `/onboarding/profile-editor`** (`app/onboarding/profile-editor/page.tsx`) — a THIN surface over #575/#576 via new adapter `lib/onboarding/profile-review.ts` (`reddi.onboarding-profile-review-view.v1`). Natural continuation of #604's intake flow (intake links to it via `intake-profile-editor-entry`); since intake drafts stay page-local, the editor reviews three fixture-backed generated drafts (complete / missing-required / inferred-warnings, all validated by the #575 validator) until draft persistence lands.
- **AC1 render surface**: capability tags + per-capability fields (shapes, risk, endpoint, auth/pricing hints), invocation (invocationAllowed:false, health not_probed), auth requirements (names/hints only), pricing + payment metadata (status/missingFields/activation:disabled), trust state, policy requirements, evidence expectations — absent fields render honestly as unavailable with blocked reasons.
- **AC2 provenance**: every field row carries the #575 five-way provenance chip; legend with per-category counts; operator-edited fields overlay as `user_provided`; blocked shown as blocked with reason codes.
- **AC3 diff view**: discovered metadata vs generated profile per field with statuses unchanged / operator_edited / inferred / generated_added / blocked.
- **AC4 required fields**: accessible labels (`label for`, `aria-required`), inline validation projected from #575 validator errors (no new rules: `private_url_blocked`, `credential_leakage_rejected` — secret input discarded, never echoed), `aria-invalid` + `aria-describedby` wiring, role=alert summary, focus moves to first invalid field, every issue carries a clear recovery action.
- **AC5 save/continue gating (#576 gate semantics)**: continue disabled while readiness is blocked unless explicitly deferred with a reason; resolved → `draft → pending_operator_approval` (operator_approval gate carried); deferred → `payment_setup_required`/`needs_provider_input` with the matching gate + readiness gate open and the reason recorded verbatim in the audit record; decision panel shows transition, `scope: local_read_model_only`, allowed next states, and the hard-false publication ledger (published_candidate stays internal per #395). Supplying the missing payment fields in the editor genuinely unblocks readiness (recovery path proven in tests + video).
- **AC6**: no publish/pay/invoke/secret paths; all fixture-backed; jest source guard asserts no fetch/XHR/WS/fs/wallet/storage surface and no async/await in the adapter; Playwright asserts zero external requests.
- **Tests**: jest `lib/__tests__/onboarding-profile-review.test.ts` (25 tests); Playwright `e2e/onboarding-profile-editor.spec.ts` (10 tests incl. keyboard/focus and intake-entry link); evidence spec `e2e/onboarding-profile-editor-evidence-385.spec.ts` (6).
- **Evidence** `docs/evidence/385/`: complete / missing-required / inferred-warnings / validation-errors at 375/768/1280 (12 PNGs) + keyboard/focus proofs (3) + continue resolved/deferred (2) + `profile-review-flow.webm` + README. Screenshots captured to `test-results/evidence-385/` and copied after the LAST Playwright run per `docs/evidence/{498,499}/README.md`; video committed instead of trace per the #506 chrome-channel precedent; captures hide the Next dev-tools `nextjs-portal` badge (triggered by the pre-existing global font-preload warning — page console verified clean).

Validation: focused jest 47/47 (new suite + intake-flow + lib/onboarding suites); Playwright editor spec 10/10; regression `e2e/onboarding-intake.spec.ts` 10/10 solo (4 flakes under full-suite load incl. the known pre-existing onboarding step-2 flakes — intake spec passes clean solo; the `/onboarding` step-2 flake alternates tests and reproduces with all #385 changes stashed); evidence spec 6/6 (last Playwright run); `npx eslint` clean on all touched files; `npx tsc --noEmit` — no errors in touched files (repo baseline unchanged at 40 lines); `npm run check:rap:naming` PASS; `npm run build` PASS (`/onboarding/profile-editor` prerenders static); `git diff --check` clean. Note: Playwright dev-server runs dirty `data/onboarding/specialist-index.json` (freshness recalculation side effect) — reverted, not committed.

RESUME FROM HERE: downstream of #385 — payment/readiness gate summary and listing preview/approval queue can consume `lib/onboarding/profile-review.ts` (gate evaluation + continue decisions); listing-surface integration with `lib/manager/marketplace-listings` deliberately deferred (owned by #394-adjacent surfaces this cycle). Draft persistence between `/onboarding/intake` and the editor is the natural next issue.

## Latest Update — Attestation/reputation bridge for Quasar and off-chain listings (#394) (2026-07-16 AEST)

Implemented the remaining #394 gap (epics #337/#388/#339; marked fully unblocked by the 2026-07-16 #500 planning pass) in isolated worktree `projects/reddi-agent-protocol-worktree-394-reputation-bridge` on branch `feat/394-attestation-reputation-bridge`, branched from `origin/main` at `b945dad3` (post-#602 tip). Audit-first outcome: ACs 1/2/4 were already merged — #439 `reddi.offchain-reputation-preview.v1`, #443/PR #601 `reddi.quasar-reputation-intent.v1` (commit/reveal/confirm/dispute intent gate on the #390 report), #442 `reddi.hosted-attestation-claim.v1`, all fail-closed. The genuine gap was AC5 (+ the composition side of AC3): no single read-model composed those three contracts into a per-listing "which reputation backing is this" explanation, and `lib/manager/marketplace-listings.ts` still carried stale copy claiming #443 hadn't landed.

Delivered (`packages/agent-protocol/src/attestation-reputation-bridge.ts`, new subpath export `@reddi/agent-protocol/attestation-reputation-bridge`):
- **`reddi.attestation-reputation-bridge.v1`** — one deterministic derivation (`deriveAttestationReputationBridge`) that composes preview + Quasar intent plan + hosted claim for a record set (`binding` + optional #390 `compatibility` + optional #442 hosted gate metadata) and classifies `status`: `hosted_attestation_backed` / `quasar_intent_fixtures` / `offchain_preview` / `insufficient_evidence` / `unverified_external` / `blocked`, with per-lane states, embedded composed records, an `evidenceSummary` linking preview/intent-plan/claim ids, and a `listingProjection` in the marketplace evidence vocabulary for off-chain listing surfaces. Binding-less external listings are marked `unverified_external` (AC3) instead of getting any reputation surface.
- **Preview-flag decision (deferred 07-07, owned here):** the frozen preview v1 guardrail `quasarInstructionBuilt: false` (literal-false type) was deliberately NOT flipped — #443 intent records are data, not built instructions, so a flag flip would be dishonest and a breaking type change. The bridge instead introduces the field the contract actually supports: a Quasar lane whose strongest state is `intent_fixtures_ready` with `instructionFlow: 'not_built'` and `quasarBackedReputation: false` (hard-typed literals). "Quasar-backed reputation" is unrepresentable in v1 by construction.
- Conformance registration per the PR #602 pattern: `attestation-reputation-bridge.test.js` composed into the existing `interop-conformance-modules` area of `scripts/run-public-conformance.mjs`, `proves` line extended.
- `lib/manager/marketplace-listings.ts`: stale "#443 must provide fixture-only intent" description on the blocked queue item refreshed to reference the merged bridge honestly (e2e-asserted phrases "Quasar compatibility pending" / "Quasar-backed claim unavailable." preserved; no type/shape change).
- README section, committed dist, 22 new tests (`tests/attestation-reputation-bridge.test.ts`): all six classifications, fail-closed gates (denied policy, unverified payment proof, failed attestation, unsafe guardrails, wrong schema, blocked #390 report, hosted inputs on non-hosted source, pending #395 gate), composed-record id/evidence consistency, determinism + frozen-input purity, all-false guardrails in every state, credential/raw-payload leak scan, and the #601-style forbidden-import/forbidden-term/never-true source guard.

Validation (offline/no-spend): package `npm test` PASS 549/0 (527→549); `npm run check:exports` PASS (95 targets); `npm run check:rap:naming` PASS; `npm run check:package:artifacts` PASS (99 + 20 files, bounded); `npm run check:conformance:public` PASS — all 9 areas incl. the extended interop area; focused lib jest (publication-eligibility, readiness-gate, approval-actions, publication-activation) 4 suites / 67 tests PASS; `npx eslint` clean on touched lib/script files; `git diff --check` clean.

Boundary note: pure fixture/read-model composition — no signing, no RPC, no program deploy, no hosted-registry write, no live payment, no marketplace publication, and no reputation mutation anywhere; every composed derivation keeps its own fail-closed gates and `buyerFacingClaimAllowed: false`. UI adoption of `listingProjection` in the manager/discovery components is a follow-up (those surfaces are owned by concurrent #381/#384 work).

RESUME FROM HERE: #395 (publication gate) can consume the bridge as its labelled read-model input. Expect this STATUS entry to conflict with the concurrent #381/#384 UI PRs; coordinator resolves.
## Latest Update — Discovery source facets for marketplace candidates (#381) (2026-07-16 AEST)

Implemented GitHub issue #381 (parent UX epic #380; deps #366/#369/#577/#593 all landed) in isolated worktree `projects/reddi-agent-protocol-worktree-381-source-facets` on branch `feat/381-discovery-source-facets`, branched from `origin/main` at `b945dad3`. Scope: `/agents` route, candidate card components, and thin lib adapters over the existing matrix/vocabulary modules — onboarding/register/manager surfaces and `packages/` deliberately untouched (a concurrent #384 intake-flow PR owns those); #382 candidate DETAIL UI intentionally not built.

Delivered:
- **Source facet vocabulary + classifiers** (`lib/discovery/source-facets.ts`, `reddi.discovery-source-facets.v1`): seven URL-addressable source classes (`rap-registry`, `ard-catalog`, `circle-x402`, `pay-sh`, `openrouter`, `local-demo`, `hosted-rap`), CSV `?source=` querystring parse/serialize (stable order, unknown ids dropped), registry-listing classifier (on-chain PDA → RAP registry; `openrouter-x402-specialist-runtime` control loop or `openrouter:` agent calls → OpenRouter; else local/demo), and #593 trust-vocabulary reuse: `mapAdapterAttestationStateToSourceTrustState` maps the legacy `externally_listed_unattested` literal (surviving only in the Circle-x402/Pay.sh adapter profiles) to `listed_untrusted` and fails closed on anything unknown; `sourceTrustStateFromLaneState` projects #577 actionability-lane states onto the #593 words. The canonical vocabulary is imported type-only (the package's ESM-style relative `.js` imports don't resolve from app webpack); runtime mirrors are `satisfies`-checked and asserted equal to `@reddi/agent-protocol/source-trust-conformance-matrix` in jest.
- **Marketplace candidate card read model** (`lib/discovery/marketplace-candidate-cards.ts`, `reddi.discovery-marketplace-candidate-cards.v1`, served by new read-only `GET /api/discovery/candidates`): thin adapters over `searchHostedMarketplaceCatalog` (#369) + `deriveHostedDiscoveryActionabilityMatrix` (#577) for hosted-rap cards (incl. blocked export records), `getStaticAgentStackReviewWorkspace` (#383) + `deriveDiscoveryActionabilityMatrix` for ARD-imported cards (malformed-connector fixtures → `failed verification`/blocked), and the Circle x402 / Pay.sh fixture catalogs (absent ingest artifacts degrade to explicit "no candidates exist" availability notes — both are absent in this environment by design, no ingest was run). All-false guardrails object; per-card trust-boundary note.
- **UI**: `/agents` rewritten around a Suspense-wrapped `useSearchParams` client tree — multi-select source pill group (`components/discovery/SourceFacetFilter.tsx`, aria-pressed, counts, focus-visible rings) composable with the now-URL-addressable task filter (`?task=`); `components/discovery/MarketplaceCandidateCard.tsx` renders source badge, media/resource type, #593 trust badge, #577 readiness badge, distinct ard-imported/untrusted/blocked treatments, and a single read-only "Why is this gated?" disclosure (no links, no other buttons — discovery cards expose no paid call, wallet action, or endpoint invocation path); `SpecialistCard` gains a source badge, resource/media line, and `data-render-state="rap-native"` / `data-trust-state` attributes. Page-level Discover≠Decide boundary banner cites `docs/DISCOVER-DECIDE-PROVE-BOUNDARIES.md`. Empty states distinguish "no candidates exist" (per-source availability notes, e.g. non-ingested Circle x402) from "filters too narrow" (clear-filters action).
- **Tests**: 28 new jest tests (`lib/__tests__/discovery-source-facets.test.ts`, `discovery-marketplace-candidate-cards.test.ts`) covering vocabulary drift-lock, URL param round-trips, classifier, adapter-literal mapping, blocked/malformed states, guardrails, determinism, and a no-legacy-literal leak scan; 10 new Playwright tests (`e2e/agents-source-facets.spec.ts`) covering URL-addressable + composable filter state, deep links, distinct hosted/ARD/RAP-native/blocked renderings, both empty-state explanations, keyboard/focus operability for pills and card actions, and the no-live-affordance boundary; evidence spec `e2e/agents-source-facets-evidence-381.spec.ts`.
- **Evidence** (`docs/evidence/381/`): 24 PNGs at 375/768/1280 (default, RAP-native, ARD-imported, blocked full+card, both empty states) + keyboard/focus proof shots + `keyboard-focus-flow.webm` (video committed instead of trace per the #506 chrome-channel precedent), README with scope/validation/boundaries. Captured to `test-results/evidence-381/` and copied after the LAST Playwright run per the #498/#499 pitfalls.
- **Infra note**: `jest.config.js` gained the standard `'^(\\.{1,2}/.*)\\.js$': '$1'` moduleNameMapper so app-side tests can import `@reddi/agent-protocol/source-trust-conformance-matrix` (ESM-style `.js`-suffixed relative imports); full suite unaffected (117 suites green).

Validation: focused + full `npx jest` PASS; `npx eslint` on touched files clean; `npx tsc --noEmit` — no errors in touched files, pre-existing test-file error count unchanged; Playwright (dedicated dev server on :3381 — port 3010 was held by the concurrent #384 worktree's server, which initially served stale pages; `PLAYWRIGHT_BASE_URL` override per config): `agents-source-facets.spec.ts` + `agents.spec.ts` 13/13, regression `navigation.spec.ts` + `manager-discovery.spec.ts` + `manager-discovery-actionability.spec.ts` pass (one pre-existing #506 evidence-capture flake under contention, passes alone/on retry), evidence spec 4/4 (last Playwright run); `npm run check:rap:naming` PASS; `npm run build` PASS; `git diff --check` clean. Known dev-only quirk handled in specs: the Suspense boundary briefly renders duplicate trees (~100–300 ms) during hydration — specs poll for the filter count to settle to 1.

Boundary note: discovery-only surfaces — no wallet signing, RPC mutation, payment, endpoint probe/invocation, publication, or trust/reputation mutation is reachable from the filter or cards; `/api/discovery/candidates` is fixture-backed and read-only. Downstream: #382 candidate detail UI can consume the same card read model + matrix; source-aware diagnostics (#344) and listing preview/approval surfaces can reuse the facet vocabulary.
## Latest Update — AI onboarding assistant guided intake flow (#384) (2026-07-16 AEST)

Implemented GitHub issue #384 (frontend UX epic #380, onboarding-assistant epic #370, workflow states #374) in isolated worktree `projects/reddi-agent-protocol-worktree-384-intake-flow` on branch `feat/384-onboarding-intake-flow`, branched from `origin/main` at `b945dad3`. Scope discipline: `/agents` routes, discovery card components, and `lib/manager/discovery` untouched (a concurrent #381 PR owns those surfaces).

Delivered:
- **Guided intake route `/onboarding/intake`** (`app/onboarding/intake/page.tsx`) — a THIN surface over the shipped #575 analyser-handoff contracts (`reddi.onboarding-*.v1`) and the #576 onboarding state machine. Every accept/reject decision comes from `validateOnboardingIntakeDescriptor` / `runOnboardingAnalyserHandoff`; successful analysis creates a `draft` read model via `createOnboardingAssistantReadModel` and renders allowed next states via `listOnboardingStateTransitions`. No new validation logic in the UI.
- **Adapter** `lib/onboarding/intake-flow.ts` (`reddi.onboarding-intake-flow-view.v1`): maps the six #384 inputs onto `OnboardingSourceKind` — endpoint URL → `manual-descriptor` (endpoint recorded, never called), AI Catalog URL → `ard-ai-catalog`, MCP card → `mcp-metadata`, OpenAPI → `openapi`, A2A card → `a2a-card`, manual seed → `manual-descriptor`; builds static-fixture descriptor candidates (deterministic `crawlTimestamp` 2026-07-06T00:00:00Z) and classifies handoff errors into UI states (`credential_leakage_rejected` → blocked-secret; URL-shaped `private_url_blocked`/`malformed_intake_descriptor` → invalid-url; other fail-closed → blocked; zero-capability inventory → empty). Pure/sync module — a jest source guard asserts no fetch/XHR/WebSocket/fs/wallet/storage surface and no async/await.
- **Consent + data-sharing boundary screen BEFORE analysis** (copy per `docs/DISCOVER-DECIDE-PROVE-BOUNDARIES.md`): what happens (local fail-closed validation, fixture-backed dry-run, provenance-tagged untrusted metadata, nothing persisted) vs what never happens (no live endpoint inspection — labelled not-yet-enabled, **#459 owns the future live adapter**; no publication/hosted write; no payment/paid invocation/wallet/RPC; no secret storage; imported content can never self-assert trust). Analysis button hard-disabled until consent.
- **All #384 states**: loading (labelled "no network request"), retryable analyser error (deterministic dry-run `unreachable` URL marker; retry recovers), invalid URL, blocked secret (input discarded immediately, never echoed — codes/paths only), empty/no-results (`empty` marker, capability-less manual seed, or bare endpoint pending #459), and analysed results (capability inventory with provenance, readiness lanes, draft state + blocking gates + next states, listing draft states, all-false guardrail grid).
- **Entry points**: `/register` banner card (`register-intake-entry`), `/onboarding` wizard header link (`onboarding-intake-entry`), `/manager` board button (`manager-intake-entry`); intake links back to `/register` + `/onboarding` from header, results, and empty states.
- **Tests**: jest `lib/__tests__/onboarding-intake-flow.test.ts` (20 tests: mapping, every state classification incl. secret-in-manual-field never echoed, determinism, guardrails all-false, source boundary guard); Playwright `e2e/onboarding-intake.spec.ts` (10 tests: all states, consent gating, secret never in DOM + input cleared, entry points, and a zero-external-data-request assertion — only the pre-existing global-layout Google Fonts stylesheet loads on any route); evidence spec `e2e/onboarding-intake-evidence-384.spec.ts`.
- **Evidence** `docs/evidence/384/`: intake/consent/loading/results/error/invalid-url/blocked-secret/empty at 375/768/1280 (24 PNGs) + `intake-flow.webm` + `blocked-input-proof.json` (deterministic validator proof over 6 malformed/secret-shaped cases, generator script committed) + README. Video committed instead of trace per the #506 chrome-channel trace-finalization hang; screenshots captured to `test-results/evidence-384/` and copied after the LAST Playwright run per `docs/evidence/498/README.md` / `docs/evidence/499/README.md`.

Validation: focused jest 22/22 (new suite + existing lib/onboarding suites); Playwright intake spec 10/10; regression `e2e/onboarding.spec.ts` + `e2e/register.spec.ts` + `e2e/manager-listings.spec.ts` + `e2e/navigation.spec.ts` — 43 passed with 2 flakes under full-suite load that pass solo (pre-existing: onboarding step-2 advance, Pay.sh listing states); evidence spec 4/4 (last Playwright run); `npx eslint` clean on all touched files; `npx tsc --noEmit` — no errors in touched files (repo baseline unchanged at 40 lines); `npm run check:rap:naming` PASS; `npm run build` PASS (`/onboarding/intake` compiles static); `git diff --check` clean.

Boundary note: zero live paths — intake performs no fetch/probe/invocation, no publication, no payment, no wallet/RPC, and no secret storage; credential-shaped input is rejected by the #575 fail-closed reasons and discarded. Dry-run URL markers (`empty`/`unreachable`) are labelled fixture-routing affordances, not network behaviour.

RESUME FROM HERE: downstream #384-blocked surfaces (RAP profile review editor, payment/readiness gate UI, listing preview/approval queue wiring) can now consume `lib/onboarding/intake-flow.ts` outcomes; live endpoint inspection remains gated on #459.
...
Delivered (`packages/agent-protocol/src/quasar-reputation-intent.ts`, new subpath export `@reddi/agent-protocol/quasar-reputation-intent`, spec `docs/QUASAR-REPUTATION-INTENT-SPEC-2026-07-07.md`):
- **`reddi.quasar-reputation-intent.v1`** — deterministic fixture-level mapping from eligible reputation/attestation record sets (validated `reddi.receipt-evidence-binding.v1` + required #390 `reddi.quasar-registry-compatibility.v1` report, optional #394 preview cross-check) to Quasar **commit/reveal/confirm/dispute INTENT records**. Plain data only, per the #589 export-conformance shape: every record carries `instructionBuilt: false` / `signable: false`; the plan carries all-false guardrails (`quasarInstructionBuilt`, `walletSigning`, `rpcCall`, `programDeploy`, `livePaymentExecuted`, `reputationMutated`, `hostedRegistryWrite`, `marketplacePublished`) and `buyerFacingClaimAllowed: false`.
- **Fail-closed gate** with structured reason codes blocking on: missing/malformed/mismatched/blocked #390 compatibility (`missing_quasar_compatibility` / `quasar_compatibility_blocked` / `compatibility_subject_mismatch`), unsafe guardrails on binding/preview/compatibility (`unsafe_live_guardrail`), policy/payment failures (`policy_denied`, `payment_preflight_denied`, `missing_payment_proof`), missing evidence (`missing_evidence`), missing attestation (`missing_attestation`), non-final and failure-final receipt states (`non_final_state_excluded`, `failure_final_excluded`, `attestation_state_excluded`), malformed source metadata (`malformed_binding`, `malformed_source_metadata`, `missing_source_ref`, `missing_intent_id`, `malformed_intent`), and preview divergence (`preview_not_ready`, `preview_mismatch`). Blocked plans carry zero intents and zero eligible lanes. Standalone `evaluateQuasarReputationIntentSourceEligibility` mirrors the #562/#589 precedent: probe-only rail-neutral receipts excluded outright, binding candidates must bridge into `reddi.receipt.v1` first, unsupported networks rejected.
- **Compact-field discipline (#390 split / #591 boundaries doc §3.3)**: intent records carry only compact program-lane args (`jobIdRef`, `role`, 1–10 `score` scaled from the draft rubric score, commitment described by the `sha256(score‖salt‖job_id‖program_id)` contract and explicitly `not_computed`); u128 encoding, salt, commitment hash, party pubkeys, and account addresses are NAMED in `deferredToInstructionBuilder`, never fabricated; rich RAP/ARD metadata travels by id in `offchainRefs` only. Lane targets document the parity-port discriminators (reputation commit=1/reveal=2; attestation confirm=2/dispute=3) with `config/quasar/deployments.json` as a pointer, not a deployment claim.
- **Lane mapping**: attested+passed → commit/reveal/confirm; rejected+consumer-disputed → commit/reveal/dispute; missing reputation draft degrades gracefully (confirm-only, `missing_reputation_draft` lane reason) without blocking.
- **Tests** (38 new, `tests/quasar-reputation-intent.test.ts`): happy-path intent shapes for all four lanes, per-gate blocking tests for every reason code above, eligibility-evaluator cases, determinism + frozen-input purity, compactness proof (rich metadata never inlined), credential/raw-payload leak scan, and a #575/#584-style forbidden-import/forbidden-term source guard (no chain-client stacks, no network/process/fs surface, no async/await, guardrails never declared true in source).

Validation (offline/no-spend): package `npm test` PASS — 527 tests / 0 fail (489→527); `npm run check:exports` PASS (93 targets); root `npm run check:rap:naming` PASS; root `npm run check:package:artifacts` PASS (97 + 20 files, bounded); `node scripts/check-quasar-boundary-guard.mjs --changed` PASS (full-scan mode has pre-existing self-referencing-pattern hits in other modules, unchanged here); `git diff --check` clean.

Boundary note: intent/fixture records only — instruction builders, salt/commitment generation, account resolution, signing, RPC, Surfpool/devnet, deploys, live payments, and reputation mutation are all explicitly OUT OF SCOPE and split into a future #441-checklist-gated issue per the issue's own boundary. Follow-up wiring (not done here, lib/ untouched by design): `lib/economic-demo/offchain-reputation-preview.ts` still reports `quasarInstructionBuilt: false` / `compatibility_pending` and `lib/manager/marketplace-listings.ts` still waits on this gate — a later issue can consume the new module to surface labelled fixture-only backed states. Conformance-suite registration in `scripts/run-public-conformance.mjs` also deferred (scripts/ owned by concurrent #501 work).

RESUME FROM HERE: expect this STATUS entry to conflict with #501's PR; coordinator resolves. Downstream: #395 may consume the gate as a labelled fixture/read-model only.
## Latest Update — Paid-workflow UI evidence pack + copy-boundary regression gate (#501) (2026-07-06 AEST)

Implemented GitHub issue #501 (final link of the #498→#499→#501 chain; unblocks closing product-UI epic #349 as product-demo ready) in isolated worktree `projects/reddi-agent-protocol-worktree-501-copy-gate` on branch `feat/501-copy-boundary-gate`, branched from `origin/main` at `878ca6c9` (post-#597/#599 tip, route selectors stable).

Delivered:
- **Copy-boundary regression gate, layer 1 (static)**: `scripts/check-paid-workflow-copy-boundaries.mjs` (npm: `check:copy:paid-workflow`) — deterministic, offline, no-browser scan of the rendered `buildBuyerPaidWorkflowRouteModel()` fixture output (the exact data `/economic-demo/paid-workflow` renders, incl. the empty-pack fail-closed variant). Fails (exit 1) on any affirmative live-claim upgrade across ten forbidden families: custody, settlement finality, production AUDD rail, default USDC auto-pay, Pay.sh production activation, mainnet settlement, hosted registry writes, live trust/reputation mutation, plus the audit-refresh Airwallex-settlement and AP2-mandate-authority overclaim cases. Asserts the six required labels (dry-run, no-spend, recorded-devnet, optional fresh-devnet, live-gated, production-disabled) and structural boundaries (full #497 16-flag grid all-false, zero spend/refunds, six blocked cases at zero/false). Patterns match only affirmative claim shapes — the route's legitimate negated copy ("no custody", "Any live Airwallex settlement claim fails closed", bare flag labels) stays green.
- **Single source of truth + drift guard**: both gate layers import `lib/economic-demo/paid-workflow-copy-boundary-terms.ts` (`reddi.economic-demo.paid-workflow-copy-boundary-terms.v1`). Every forbidden family cites an exact anchor in `docs/DISCOVER-DECIDE-PROVE-BOUNDARIES.md` (#452, the fail-closed copy authority); the script whitespace-normalizes that doc and fails closed if any anchor disappears, so the gate must be re-derived when the authority doc changes.
- **Negative control**: built-in self-test on every run injects all ten `injectionExample` phrases into a cloned model and fails the gate if any goes uncaught; `--negative-control` injects a forbidden phrase into the live model and exits 1 (verified locally: clean run exit 0, negative-control exit 1).
- **Copy-boundary regression gate, layer 2 (DOM)**: `e2e/economic-demo-paid-workflow-copy-gate.spec.ts` — same shared forbidden/required lists asserted against the rendered DOM of `/economic-demo/paid-workflow`, plus the 16 flag labels + "All live flags false", the Discover/Decide/Prove authority reference, negated never-claims copy, and the zero-buttons invariant.
- **CI wiring**: new `paid-workflow-copy-boundary-gate` job in `.github/workflows/rap-package-guard.yml` (root `npm ci` → gate run → negative-control run that must fail), registered alongside the #353 public-conformance runner and #449 artifact guard; workflow `paths` extended to the paid-workflow surfaces (`app/economic-demo/paid-workflow/**`, `lib/economic-demo/**`, the gate script, the paid-workflow e2e specs, and the boundaries doc). Gate row added to `docs/RAP-V0.1-RELEASE-CHECKLIST.md` §2.
- **Evidence pack** (`docs/evidence/501/`): per-state element screenshots — quote, ledger, timeline, result, receipt, evidence, blocked — at mobile-390/tablet-820/desktop-1440 plus full-page captures (24 PNGs), and `buyer-workflow-copy-gate-flow.webm` covering the quote → ledger → timeline → result → receipt/evidence → blocked flow. Captured by `e2e/economic-demo-paid-workflow-evidence-501.spec.ts` into `test-results/evidence-501/` and copied after the run (webpack dev-watcher race + test-results wipe pitfalls per `docs/evidence/498/README.md` / `docs/evidence/499/README.md`; video committed instead of trace per the #506 chrome-channel trace-finalization hang). README documents the deterministic artifact path.

Validation:
- `node scripts/check-paid-workflow-copy-boundaries.mjs` PASS (exit 0: 10/10 doc anchors, 0 violations across 484 rendered strings, 6/6 labels, 16/16 flags false, 10/10 self-test injections caught); `--negative-control` exits 1 as required.
- `npx jest lib/__tests__/economic-demo-buyer-paid-workflow-route-model.test.ts` 16/16.
- `PLAYWRIGHT_BROWSER_CHANNEL=chrome npx playwright test e2e/economic-demo-paid-workflow-copy-gate.spec.ts e2e/economic-demo-paid-workflow.spec.ts` 9/9 (2 new + 7 existing); `e2e/economic-demo-public-proof.spec.ts e2e/economic-demo.spec.ts` 5/5; evidence capture spec 4/4 (last Playwright run).
- `npx eslint` clean on all touched files; `npx tsc --noEmit` pre-existing repo error count unchanged (40 lines, none in touched files).
- `npm run check:rap:naming` PASS; `npm run build` PASS; `git diff --check` clean.

No live paths introduced: the gate script reads local fixtures and one local doc file; the route still exposes zero buttons and all boundary flags stay false.

RESUME FROM HERE: #501 closes the #498→#499→#501 chain — epic #349 can be recommended for product-demo-ready close-out once the #501 PR merges. Future paid-workflow copy changes must keep `paid-workflow-copy-boundary-terms.ts` in sync with `docs/DISCOVER-DECIDE-PROVE-BOUNDARIES.md` (the gate fails closed on anchor drift by design).

## Latest Update — Paid-workflow ledger + execution timeline evidence states (#499) (2026-07-06 AEST)

Implemented GitHub issue #499 (product UI epic #349 / #335; step 2 of the #498→#499→#501 chain) in isolated worktree `projects/reddi-agent-protocol-worktree-499-ledger-timeline` on branch `feat/499-paid-workflow-ledger-timeline`, branched from `origin/main` at `f539cc74` (post-#597/#498 tip). Extends the #498 route/model/spec in place — route file ownership was settled for #499 by the #498 merge.

Delivered:
- **Budget ledger** (`ledger` on `reddi.economic-demo.buyer-paid-workflow-route-model.v1`, rendered as `paid-workflow-ledger` on `/economic-demo/paid-workflow`): buyer budget, per-specialist reserved budgets, attestor/proof budget, orchestrator fee/margin, protocol rail fee (bps), and swap allowance from the `economicDemoScenarios` webpage `budgetLedger` fixture; spent-to-date and remaining as authoritative zeros from the #582/#564 rehearsal real metering; refund state from the AUDD payment-plan preflight (`no_charge_on_failure` / `manual_review_fixture_only`); real settlement cost explicitly **unavailable** (ledger-reconciliation `real_devnet_receipt_verifier: not_implemented` proof layer); blocked-cases spend explicitly **blocked** at 0 USDC referencing all six fail-closed cases. Allocations reconcile in integer micro-USDC to the quote total — mismatch fails the whole model closed (`ledger_allocation_mismatch`), as do untraceable rows (`ledger_row_untraceable`) and a missing scenario ledger (`budget_ledger_source_missing`). Helper `reconcileBuyerPaidWorkflowLedgerAllocation` exported for tests.
- **Execution timeline** (replaces the #498 placeholder; `paid-workflow-timeline`): the nine #499 milestones — request, quote, policy decision, execution, result, receipt, evidence, attestation preview, reputation preview. Seven workflow milestones map to the recorded #582/#564 dry-run rehearsal steps and carry `devnet_proof_metadata` labels referencing `docs/DEVNET-REFERENCE-RUN-564.md`; execution is `planned_no_live_execution` with the zero-downstream-calls metering ref; previews are `preview_only` with `simulated` state labels, and the reputation preview is explicitly `no_public_ref_preview_only` (the "explicitly marked unavailable" rule exercised on the timeline). Untraceable or missing milestones fail closed (`timeline_milestone_untraceable` / `timeline_milestone_missing`).
- **Blocked spend states (criterion 3)**: every blocked case (unsupported network, unsupported asset/network, malformed receipt, policy denied, #588 probe-only cap, live-path overclaim) now carries `spendState` (spent "0", refunds 0, spending/mutation false, boundary flags all-false) and renders a spend-state line on its card; live-gated and production-disabled sections unchanged and still claim nothing.
- **16-flag hard-boundary grid (criterion 4)**: new `hardBoundaryFlags` = 12 fixture-pack flags + the four #497 contract-only flags (`productionAuddRail`, `defaultUsdcAutoPay`, `mainnetSettlement`, `payShProductionActivation`), all false, rendered in the grid; copy stays negation-only per `docs/DISCOVER-DECIDE-PROVE-BOUNDARIES.md`.
- **Tests**: route-model jest suite extended to 16 tests (ledger categories/reconciliation/traceability, timeline milestones/labels/refs, blocked spend states, 16-flag grid, doctored-allocation and missing-scenario fail-closed paths). Playwright spec extended to 7 tests (three viewports with ledger+timeline assertions, a dedicated ledger/timeline traceability test, copy modes, boundary states incl. per-card spend-state and the four contract-only flags, scroll-through flow; still asserts zero buttons).
- **Evidence** `docs/evidence/499/`: mobile/tablet/desktop PNGs + `buyer-ledger-timeline-flow.webm` + README (trace omitted per the #506/#498 chrome-channel precedent). New operational note: any subsequent `npx playwright test` run wipes `test-results/`, so the evidence-capture run must be the LAST Playwright invocation before copying artifacts out.

Validation: focused jest 5 suites / 32 tests PASS; eslint clean on touched files; `tsc --noEmit` — no errors in touched files (repo-wide pre-existing error count unchanged at 40 lines); Playwright route spec 7/7 PASS; `e2e/economic-demo-public-proof.spec.ts` 4/4 and `e2e/economic-demo.spec.ts` 1/1 PASS individually; `npm run check:rap:naming` PASS; `npm run build` PASS; `git diff --check` clean.

Boundary note: zero live paths — the ledger reports zero spend/refunds and an explicitly unavailable settlement cost; the timeline reports no live execution; no wallet, RPC, provider call, paid request, Pay.sh activation, hosted-registry write, publication, or trust/reputation mutation is introduced or reachable. Scope discipline: packages/ untouched (#470 owns it concurrently); the automated copy-boundary regression GATE is #501's scope, not built here.

RESUME FROM HERE: #501 (evidence pack + copy-boundary regression gate) follows and consumes this route's ledger/timeline surface. Expect this STATUS entry and the #470 PR to conflict; coordinator resolves.

## Latest Update — Repo explorer evidence manifest for static fixture ingestion (#470) (2026-07-06 AEST)

Implemented GitHub issue #470 (FastContext-style repo explorer evidence manifest; epic #468, roadmap #400) in isolated worktree `projects/reddi-agent-protocol-worktree-470-explorer-manifest` on branch `feat/470-repo-explorer-evidence-manifest`, branched from `origin/main` at `ed4adae7` (post-#596 tip). Touches only `packages/agent-protocol` and this file — no app/, components/, e2e/, or lib/ changes (a concurrent #498 PR owns those surfaces).

Delivered (`packages/agent-protocol/src/repo-explorer-evidence-manifest.ts`, new subpath export `@reddi/agent-protocol/repo-explorer-evidence-manifest`):
- **Typed RAP-owned contract** `reddi.repo-explorer-evidence-manifest.v1` — self-contained zero-import module per the #575/#584/#585 pattern (mirrors the OpenClaw-side `openclaw.repo-explorer-evidence.v0.1` shape, but RAP-owned, not an import). Captures repo/source https URL + resolved commit SHA (7–64 hex; branch names/HEAD fail closed), hard-coded `external_untrusted` trust boundary, task-specific exploration query, verbatim read-only explorer contract statement, file paths + 1-based line ranges, short relevance reasons, generated/noisy exclusions, and open questions. Reuses the repo-wide info/warning/blocked severity vocabulary.
- **Fail-closed validation** (`validateRepoExplorerEvidenceManifest`) with 22 structured reason codes: unsafe paths (absolute, `..` traversal, `file://`/URI schemes, drive prefixes, `~` expansion, percent-encoded traversal), malformed paths (backslashes, control chars, empty/`.` segments), invalid line ranges (zero/negative/reversed/non-integer), empty evidence, missing relevance reasons, self-contradictory excluded-path citations, non-https/credentialed source URLs, unresolved commits, self-asserted trust, non-read-only explorer contracts, and refused live operations (`operation_not_permitted`). Blocked verdict ⇒ `manifest: null`. Generated/noisy citations (node_modules, dist, lockfiles, minified/map/snap) are accepted-but-flagged (`generated_path_cited`).
- **#575 snapshotRef bridge** (`attachRepoExplorerEvidenceToSnapshot`): accepted manifests attach as provenance BY REFERENCE — the attachment `source` block is a structural match for `OnboardingIntakeSource` and is accepted verbatim by `validateOnboardingIntakeDescriptor` under `sourceKind: 'static-agent-stack-snapshot'` (proven in tests); per-citation `repo-explorer-evidence:<id>#<path>:L<a>-L<b>` refs satisfy the provenanced-field `verified` ⇒ non-empty-evidence-refs rule. No full-repo ingest, ever (`fullRepoIngested: false` hard-coded).
- **Projections** (#593 registryProjection/diagnosticsProjection pattern, structural matches into the shipped agent-stack-fixtures vocabulary): `capabilityInventoryProjection` (#403 provenance block + `untrusted_public_text` + localization hints), `connectorDiagnosticsProjection` (#404 diagnostic shape, lane `repo_explorer_evidence`), `riskTaxonomyProjection` (#421 nine-category union via deterministic path/term rules; flags-for-review only, never auto-blocks), `draftReadinessProjection` (#405; never emits `ready` — external_untrusted always needs operator review), `operatorReviewProjection` (#406; publication disabled, imported content untrusted), plus the `projectRepoExplorerEvidence` bundle (fail-closed nulls/empties when blocked).
- **Docs**: package README section stating the boundary — explorer output is localization evidence, not approval to install/run/adopt; review-only `reviewBoundary` + all-false guardrails on every report.
- 15 canonical fixtures (`repoExplorerEvidenceManifestFixtures`) on `example.invalid` hosts; committed dist; subpath export wired.

Validation (offline/no-spend): `packages/agent-protocol` `npm test` PASS — 489 tests / 0 fail (448 pre-existing + 41 new covering every fail-closed case, happy path, bridge, and all five projection shapes); package `npm run check:exports` PASS (91 targets); root `npm run check:rap:naming` PASS; root `npm run check:package:artifacts` PASS (95 + 20 files, bounded); `git diff --check` clean.

Boundary note: the module DESCRIBES exploration evidence — it never fetches, clones, crawls, ingests, executes, or installs anything; fixtures only; zero imports in the core module.
## Latest Update — No-spend buyer paid-workflow route shell (#498) (2026-07-06 AEST)

Implemented GitHub issue #498 (product UI epic #349 / #335) in isolated worktree `projects/reddi-agent-protocol-worktree-498-buyer-route-shell` on branch `feat/498-no-spend-buyer-route-shell`, branched from `origin/main` at `ed4adae7` (post-#596 tip).

Delivered:
- **Route shell** `app/economic-demo/paid-workflow/page.tsx` (`/economic-demo/paid-workflow`, statically prerendered): buyer journey quote → budget summary → execution/timeline placeholder (clearly labeled, deferred to #499) → result summary → receipt/proof summary (refs/hashes only) → evidence refs → attestation/reputation previews, plus the evidence boundary states and the six copy modes (no-spend / dry-run / simulated / recorded-devnet / live-gated / production-disabled) with copy checked against `docs/DISCOVER-DECIDE-PROVE-BOUNDARIES.md` (#591) and the #497 copy boundary matrix.
- **Adapter** `lib/economic-demo/buyer-paid-workflow-route-model.ts` (`reddi.economic-demo.buyer-paid-workflow-route-model.v1`): maps the #497 state contract's 11 route states onto existing surfaces only — #457 fixture pack, #417 public proof data, #582 x402 reference rehearsal (recorded-devnet metadata + live gate), #587 `AIRWALLEX_RAIL_SUPPORT_STATE_MATRIX` (real unsupported-rail/probe-only rows). Fails closed (discriminated `fail_closed` result with reason code) on empty case list, schema mismatch, missing happy path, boundary-flag drift, non-dry-run rehearsal, or missing rail matrix rows. No new proof shape.
- **Fail-closed states rendered**: empty contract (real builder output for an empty case list), all six blocked rail-neutral cases — including the post-#588 `airwallex_webhook_probe_only_cap` (probe-only receipts can never mint a reddi.receipt.v1 binding) — unsupported second-rail support states, live-gated-only (approval required, no run button), production-disabled, and the 16-flag all-false hard-boundary grid.
- **Tests**: `lib/__tests__/economic-demo-buyer-paid-workflow-route-model.test.ts` (8 tests: state mapping, blocked-case classification, empty/malformed/drift/missing-matrix fail-closed paths, copy modes) and `e2e/economic-demo-paid-workflow.spec.ts` (6 tests: three viewports, copy modes, all boundary states incl. probe-only cap, scroll-through flow capture; asserts the route exposes zero buttons).
- **Post-#588 test repairs on consumed surfaces**: `economic-demo-paid-workflow-proof-ui-fixtures.test.ts` and `economic-demo-public-proof-page-data.test.ts` expected 5 blocked cases and failed on `origin/main`; updated to include `airwallex_webhook_probe_only_cap` (6 blocked).
- **Evidence** `docs/evidence/498/` (per the #506 precedent): mobile/tablet/desktop PNGs + `buyer-workflow-flow.webm` + README. Playwright trace not committed — trace finalization hangs under `PLAYWRIGHT_BROWSER_CHANNEL=chrome` (documented in `docs/evidence/506/README.md`). New pitfall documented: writing screenshots into `docs/`/`artifacts/` mid-run triggers a webpack dev-watcher invalidation race for the next navigation; the spec captures to `test-results/` and evidence is copied afterwards.

Validation: focused jest 4 suites / 24 tests PASS; eslint clean on touched files; `tsc --noEmit` — no errors in touched files (repo-wide pre-existing error count unchanged at 40); Playwright route spec 6/6 PASS; `e2e/economic-demo-public-proof.spec.ts` 4/4 and `e2e/economic-demo.spec.ts` 1/1 PASS individually; `npm run check:rap:naming` PASS; `npm run build` PASS (route listed static); `git diff --check` clean. No BDD scenarios added.

Boundary note: zero live paths — no wallet, RPC, provider call, paid request, Pay.sh setup, hosted-registry write, marketplace publication, or trust/reputation mutation is introduced or reachable; the route renders deterministic fixture/dry-run/recorded-rehearsal metadata only and fails closed otherwise.

RESUME FROM HERE: #499 (ledger/timeline polish) owns the execution/timeline placeholder and richer budget rows; #501 (evidence pack + copy regression gate) follows. Route file ownership is now settled for #499.
...
Audit first — existing vs. gap (compose, don't rebuild): quickstart (criterion 1) already satisfied by #357 (`README.md#ard-no-spend-quickstart`, `npm run example:ard:no-spend` — re-verified from clean `npm ci`, runs offline/no-spend). Example specialist workflow (criterion 2) already satisfied by `examples/ard-no-spend-demo.mjs` — its output demonstrates 402 challenge (`reddi.payment-challenge.v1`), policy decision, receipt, evidence binding, attestation/reputation preview, and six failure states. Fixture coverage (criterion 4) fully present across `src/fixtures.ts` (happy path, policy denial, missing proof ref, unsupported network/asset, malformed receipt, credential leakage), the demo `failures` block (malformed challenge, missing evidence, missing payment setup, unsupported rail/network, unsafe metadata), rail-neutral proof-chain blocked cases, and AUDD proof-metadata/no-custody labels (`devnet_proof_metadata` state label + `boundaries` custody flags + `custodyClaim: false` in rail-neutral receipts) — **zero new fixtures needed**. OSS-vs-hosted docs (criterion 5) satisfied by #591 boundaries doc + #592 checklist §8/§9 — linked, not duplicated. Genuine gaps: no single consolidated conformance command (criterion 3) and no single public doc tying 1–5 together (criterion 6).

Delivered:
- **Consolidated conformance suite** (`scripts/run-public-conformance.mjs`; package script `npm run conformance` in packages/agent-protocol, root script `npm run check:conformance:public`): deterministic, offline runner that COMPOSES the existing compiled per-module suites into 8 labeled areas — receipt-shape, policy-decision-shape, source-metadata, challenge-handling, evidence-binding, secret-leakage-rejection, quickstart-no-spend-workflow, interop-conformance-modules — plus the #449 packed-artifact secret/content guard as a ninth step. No new checks written. Includes a drift guard (fails if any composed suite file is renamed/removed) and `--skip-build` for CI reuse of prior build output. Negative control verified: removing a composed suite file → exit 1.
- **CI wiring**: sixth step "Public conformance suite (#353)" appended to `rap-package-guard.yml` (runs `--skip-build` after build/test on every PR touching packages/agent-protocol/**).
- **Public tying doc** (`docs/RAP-V0.1-DEVELOPER-QUICKSTART-AND-CONFORMANCE.md`): single external-developer entry point — quickstart commands, example-workflow surface table (what the demo proves per #353 criterion 2), conformance area table, fixture coverage matrix (required case → where it lives), OSS-vs-hosted links (#591/#592, link-only), and full verification path pointer to checklist §2/§7.
- **Docs polish**: package README gained a "Public Conformance Suite" section (before Local Validation); release checklist §7 gained the `npm run conformance` command; checklist §10 #353 row updated to CLOSED-with-landing-location and the launch gate reduced to #450-only.

Validation: `npm run check:conformance:public` PASS (all 9 areas); `--skip-build` path PASS; agent-protocol `npm test` PASS (448 tests / 0 fail); `npm run example:ard:no-spend` PASS from fresh `npm ci`; `npm run check:exports` PASS; `npm run check:rap:naming` PASS; `npm run check:package:artifacts` PASS; `git diff --check` PASS.

Boundary note: no live calls, no spend, no network, no wallet/RPC, no secrets — every added command is fixture-only and deterministic. The runner script lives repo-side (not shipped in the tarball; `files` whitelist unchanged, pack contents unchanged).

## Latest Update — Source/trust conformance matrix for auth.md and ARD metadata (2026-07-06 AEST)

Implemented GitHub issue #450 (launch-gate blocker; epics #336 / #363) in isolated worktree `projects/reddi-agent-protocol-worktree-450-source-trust-matrix` on branch `feat/450-source-trust-conformance-matrix`, branched from `origin/main` at `b4108668` (post-#592 tip). Touches only `packages/agent-protocol`, `docs/conformance/source-trust-matrix/README.md`, and this file.

Audit first — existing vs. gap: the CI `source-conformance-matrix` lane (`scripts/run-source-conformance-matrix.sh`, `npm run test:source:matrix`) is a per-source-**adapter** smoke matrix (openclaw/hermes/pi/circle-x402/pay-sh) — same name, different thing, untouched. `provider-trust.ts` (#343 shape) already covered claimed/verified/unverified/failed_verification + malformed-trust/credential-leakage for AI Catalog only; `source-diagnostics.ts` (#344) already separates relevance from trust/policy/payment/reputation lanes; `discovery-actionability-matrix.ts` (#506) has blocked/needs_human_review as UI lane states. Genuine gaps: no auth.md metadata handling anywhere on main; no `trusted`/`listed_untrusted` states as source-trust classification; no anonymous-write-scope / unsupported-credential-type / unsupported-identity-assertion / high-relevance-blocked cases; no single matrix binding BOTH source kinds as untrusted-until-gated with #343/#344-consumable projections.

Delivered — `packages/agent-protocol/src/source-trust-conformance-matrix.ts` (`reddi.source-trust-conformance-matrix.v1`, exported as `@reddi/agent-protocol/source-trust-conformance-matrix`), pure/fixture-only, fail-closed, no live calls:
- **All 7 #450 states** (`trusted` / `listed_untrusted` / `claimed` / `unverified` / `failed_verification` / `blocked` / `needs_human_review`) with `listed_untrusted` as the mandatory ingress state for BOTH auth.md and ARD/AI Catalog metadata (`entryState` constant + finding on every row).
- **All 7 #450 required cases** fixture-covered (malformed metadata × both source kinds, credential leakage, anonymous write scope × both source kinds, unsupported credential type, unsupported identity assertion, missing trust evidence, high-relevance-but-blocked at relevance 0.99); `buildSourceTrustConformanceMatrix().coverage.complete` asserts full state/case/source-kind coverage so regressions fail the suite.
- **Vocabulary reuse, no new words where existing fit**: provider-trust reason codes reused verbatim; severities are the repo-wide `info`/`warning`/`blocked` (#504 okf-conformance / #344); states mirror #506 lane-state overlap; auth.md registration/credential vocabulary aligns with the auth.md discovery lane (`agent-provider`/`email-verification`/`anonymous`, `access_token`/`api_key`); identity-assertion allowlist marked DRAFT/illustrative (fail-closed behavior is the contract).
- **#343/#344 consumability**: `registryProjection` maps every state onto `ProviderTrustVerificationStatus`, carries reason codes, marks blocked rows `registryEligible: false`, and includes the normalized `ProviderTrustRecord` for valid ARD rows; `diagnosticsProjection` uses the `source-diagnostics.ts` lane + severity vocabulary with a `relevance_only_not_trust` capability-match message on every row.
- **Boundary alignment**: references `docs/DISCOVER-DECIDE-PROVE-BOUNDARIES.md` (#452, merged today) rather than duplicating it; self-asserted "verified" status and verified-with-residual-warnings both route to `needs_human_review`, never auto-trust.
- Docs: `docs/conformance/source-trust-matrix/README.md` (states/cases tables, projections, boundary section, run command, explicit disambiguation from the adapter-smoke CI lane) + a package README section. Tests: `packages/agent-protocol/tests/source-trust-conformance-matrix.test.ts` (11 tests).

Validation: agent-protocol `npm test` PASS (448 tests / 0 fail, 11 new); `npm run check:exports` PASS (89 targets incl. new subpath); `npm run check:artifacts` PASS (91-file pack); `npm run check:rap:naming` PASS; `git diff --check` PASS. No BDD docs changed, so `npm run test:bdd:index` not required; the adapter-smoke `test:source:matrix` lane is untouched by this change (its trigger paths don't include packages/agent-protocol).

Boundary note: fixtures only — no live auth.md fetch, no catalog crawl, no hosted-registry write, no trust/reputation mutation, no payment, no live evidence required (#393 vocabulary not needed). auth.md AND ARD/AI Catalog provider metadata both enter as untrusted-until-gated; discovery relevance never influences trust classification.

## Latest Update — RAP v0.1 release checklist + package artifact guard (2026-07-06 AEST)

Implemented GitHub issue #449 in isolated worktree `projects/reddi-agent-protocol-worktree-449-release-checklist` on branch `feat/449-v01-release-checklist`, branched from `origin/main` at `7a4337ab` (post-#591 tip).

Delivered:
- **Release checklist doc** (`docs/RAP-V0.1-RELEASE-CHECKLIST.md`): deterministic release gates table (release:dry-run, exports guard, artifact guard, naming guard, #512 clean-checkout smoke), required package contents, README coverage, changelog/license/security/contributing status table (two honest open items: per-package LICENSE file in tarball, package-level v0.1.0 release notes — root CHANGELOG.md is app-focused/stale), artifact exclusion classes, OSS/self-hosted path (§8) documented separately from the hosted RAP marketplace/facilitator path (§9, explicitly out of v0.1 scope, #395-owned), explicit no-spend local verification path (§7: quickstart + package tests + conformance-fixture + release:dry-run commands), and a live checklist-inputs table for #353 (open — remaining conformance-suite packaging) / #357 ✅ / #416 ✅ / #417 ✅ / #450 (open — launch-gate blocker) / #451 ✅ / #452 ✅ (closed today via PR #591). Launch gate = all §2 gates PASS + #450 + remaining #353 scope.
- **Package artifact guard** (`scripts/check-package-artifact-contents.mjs`, root npm script `check:package:artifacts`, per-package `check:artifacts`): standalone deterministic `npm pack --dry-run --json` inspection for the v0.1 candidate set (`@reddi/agent-protocol`, `@reddi/x402-solana`; sendai/eliza stay deferred). Fails on secrets (.env/.npmrc/keys/wallets/keypairs/pem), generated noise (node_modules/, **dist-tests/**, **test-results/**, artifacts/, coverage/, logs, tsbuildinfo, nested tgz, .DS_Store), private research (research/, ingests/), and unrelated app/repo files (app/, components/, config/, e2e/, tests/, programs/, third_party/); also enforces manifest hygiene (files whitelist, license declared, not private, required package.json/README.md/dist contents). Gap it closes: the #573 `rap-package-guard` CI lane only proved exports resolution, and pack-content rules lived only inside the heavyweight #512 smoke (which also lacked dist-tests/ and test-results/ patterns).
- **Wiring**: guard appended to `rap-package-guard.yml` as a fifth step (runs on every PR touching packages/agent-protocol/**), to agent-protocol `release:dry-run` + `prepublishOnly`, and to x402-solana `release:dry-run`. `@reddi/x402-solana` gained its missing `"license": "MIT"` declaration (repo LICENSE is MIT; guard enforces the field). `.gitignore` gained `packages/agent-protocol/dist-tests/` (generated test output was untracked-but-unignored).
- **First executed dry-run pass recorded** (checklist §11, date-stamped 2026-07-06): agent-protocol `release:dry-run` PASS — 437 tests / 0 fail, exports guard OK (87 targets at run time), artifact guard OK (89-file pack, ~234 kB, only package.json/README/dist/examples), plus negative control (injected `dist/.env` → guard FAIL with named reason).

Validation: agent-protocol `npm run release:dry-run` PASS; `npm run check:package:artifacts` PASS (both candidates); per-package `check:artifacts` PASS; negative-case test PASS (deterministic failure + exit 1); `npm run check:rap:naming` PASS; `git diff --check` PASS. x402-solana full dry-run not re-run here (its jest suite + rebuild churns committed dist); its pack contents are covered by the root guard run.

Boundary note: no npm publish (dry-run only), no live calls, no network, no wallet/RPC, no secrets in any output. Launch-gate inputs #450 and remaining #353 scope stay open — the checklist exists and is executable regardless, and says so.
...
Delivered:
- **Adapter promotion** (`packages/agent-protocol/src/ap2-mandate-ingestion.ts`, schema stays `reddi.ap2-mandate-ingestion.v1`): `AP2_MANDATE_INGESTION_DRAFT` is now `false` and results/refs/bindings emit `draft: false` — the RAP-side contract (ingestion rules, reason codes, fail-closed lanes, composition semantics, receipt-binding contract) is spec'd and frozen for v1. External-standard honesty preserved separately (#589 convention): every result carries `externalStandard: { name: 'Google AP2 (Agent Payments Protocol)', status: 'external-draft-standard', fieldShapesVerified: false, signatureVerification: 'fixture-asserted', governanceLineageVerified: false, settlementRail: false }`, every AP2/FIDO/Visa/Mastercard field keeps its `(DRAFT/unverified — …)` tag, and the `AP2_RAIL_SUPPORT_STATE_MATRIX` rows deliberately stay `draft: true` / `confidence: 'low'` — they describe the EXTERNAL mandate vocabulary, which remains unverified. New machine-readable tables: `AP2_MANDATE_FIELD_PROVENANCE` (13 rows: target, AP2 source path, rap-native vs ap2-draft-interface confidence, lossy notes) and `AP2_UNSUPPORTED_FIELDS` (9 fail-closed gaps: live VDC verification, scheme key material, PANs/card instruments, settlement/custody, cross-unit conversion, mandate lifecycle → #338, authority widening, mandate contents on receipts, open-mandate authorization).
- **New fail-closed ingestion lanes**: `unsupported_mandate_type` (valid VDC envelope + unknown mandate stage — never coerced) and `signature_material_rejected` (signature/JWS/proof-shaped material outside the single opaque `vdc.signatureB64` slot — RAP does no key handling; guardrails gained `keyMaterialHeld: false`). Amount-precision guard: decimal AP2 fixture amounts normalize losslessly (≤2 decimals) into integer ap2-fixture-centiunits (`ap2FixtureCentiUnits`) so the BigInt-based buyer-authority gate can actually compare them (the draft's decimal caps made the gate throw→deny on everything, i.e. the composed policy was unenforceable); higher precision fails closed, and this is within-lane only, never cross-rail conversion.
- **Policy-gate composition — a mandate never widens local authority** (`composeAp2MandateWithLocalPolicy(localPolicy, ingestion)`): currency and merchant must ALREADY be permitted by the local `reddi.buyer-authority-policy.v1` (else blocked `mandate_currency_not_permitted_locally` / `mandate_merchant_not_allowlisted_locally`); a missing local AP2-lane cap never becomes unlimited authority (blocked `local_cap_missing_for_mandate_currency`); composed cap = MIN(local, mandate) with wider/incomparable mandate caps recorded (`mandate_cap_wider_than_local_cap`) and ignored; expiry takes the earlier bound; operator approval only escalates (`operator_approval_escalated`, e.g. human-not-present mandates); mode/receipt/evidence/refund/support-state constraints carried verbatim from local. Probe-only/rejected ingestions never compose (`mandate_not_authorizing`); the composed policy must itself pass `validateBuyerAuthorityPolicy` and is proven enforceable by the real `evaluateBuyerAuthorityPolicy` gate.
- **Receipt binding hardened** (`bindMandateToReceipt` now returns a fail-closed result): only authorizing `ap2_mandate_fixture` refs bind; probe-only refs never bind (`probe_only_ref_not_bindable` — a probe-only mandate authorized nothing; probe/dry-run semantics respected), rejected refs never bind, signature-material-bearing or malformed refs never bind, and the bound receipt must still pass `validateReddiReceipt`. Binding stays hash + type + support-state only — never contents or signature material.
- **No-live conformance surface** (`packages/agent-protocol/src/ap2-mandate-conformance.ts`, `reddi.ap2-mandate-conformance.v1`, exported as `@reddi/agent-protocol/ap2-mandate-conformance`, #589 style): `verifyAp2IngestionAgainstMandate` recomputes every derived field AND the mandate hash from the source — tampering with either side (e.g. cart total edited after ingestion) fails the named `mandate_hash_recomputes` check. `listAp2ConformanceFixtures()` / `runAp2MandateConformanceSuite()` — 20 deterministic fixtures: all issue-required lanes (valid, tampered, expired, over-cap vs local policy, unsupported type, signature-material fixture, settlement-finality claim, PAN, unsupported currency, probe-only) + 6 composition never-widen cases + 4 binding cases.
- **Spec doc**: `docs/AP2-MANDATE-INGESTION-SPEC-2026-07-06.md` — positioning (AP2 = authorization layer, not settlement), signature/key hard boundary, lane tables, composition invariant, binding contract, conformance contract.
- Tests: `tests/ap2-mandate-conformance.test.ts` (9 new node:test tests — suite green + deterministic, coverage of all 20 fixture lanes, tamper fail on both sides, offline/no-async/no-crypto source guards over both modules, unverified-tag + boundary-statement guards, no-signature-leak sweep) and `tests/ap2-mandate-ingestion.test.ts` updated + extended for the promoted shape (22 tests: externalStandard block, provenance/unsupported tables, new lanes, fail-closed binding, composition end-to-end through the real gate incl. allowed-within-cap case). Barrel + package exports wired, rebuilt dist committed.

Validation:
- `cd packages/agent-protocol && npm test` PASS (437 tests, incl. 16 new/updated; exports-resolution guard in-suite).
- `npm run check:exports` PASS (87 targets). `npm run check:rap:naming` PASS. `git diff --check` PASS.

Boundary note: no live calls, no network, no wallet/RPC, no key handling, no cryptographic signature verification (fixture-asserted only — no real verification of Visa/Mastercard/FIDO material), no custody, no settlement-finality claim anywhere, fixtures only. Frozen v1 unions untouched (buyer-authority policy consumed as-is; AP2 constraints live on the RAP-internal `ap2-authorization-fixture` lane). AP2 field shapes remain draft/unverified external references; mandate lifecycle (revocation/supersession) routes to #338.

## Latest Update — RAP → ERC-8004 export hardened: spec promotion + round-trip conformance fixtures (2026-07-06 AEST)

Implemented GitHub issue #562 (promoting the PR #570 DRAFT "RAP receipt → ERC-8004 registry export" to a complete, spec'd, fixture-proven surface) in isolated worktree `projects/reddi-agent-protocol-worktree-562-erc8004-hardening` on branch `feat/562-erc8004-export-hardening`, branched from `origin/main` at `87d6fe4c` (post-#588 tip). Touches only `packages/agent-protocol`, `docs/ERC8004-EXPORT-SPEC-2026-07-06.md`, and this file.

Delivered:
- **Spec promotion** (`packages/agent-protocol/src/erc8004-export.ts`, schema stays `reddi.erc8004-export.v1`): `ERC8004_EXPORT_IS_DRAFT` is now `false` and bundles emit `draft: false` — the RAP-side mapping (sources, exclusion rules, reason codes, round-trip contract) is spec'd and frozen for v1. External-standard honesty is preserved separately: every bundle carries `externalStandard: { name: 'ERC-8004 (Trustless Agents)', status: 'external-draft-standard', fieldShapesVerified: false, deploymentClaim: false }`, and every ERC-8004-side field keeps an `(unverified — ERC-8004 draft standard)` tag (Airwallex-style convention). New machine-readable tables: `ERC8004_EXPORT_FIELD_PROVENANCE` (per-field source path + rap-native vs erc8004-draft-interface confidence + lossy notes, all three registries + crossReference) and `ERC8004_UNSUPPORTED_FIELDS` (fail-closed documentation: minting/agentId, EVM feedback auth, registry lifecycle, negative feedback, full rubric, raw evidence, deployments, trust import). `ERC8004_DOCUMENTED_CHAINS` records the issue-text chains (Ethereum, Base, Polygon, Monad, BNB Chain) with `deploymentClaim: false` / `verified: false` on every row (Monad CAIP-2 left null — unconfirmed offline); chain hints are validated as CAIP-2 `eip155:*` fixture placeholders and otherwise fail closed (`unsupported_chain_hint`).
- **Exclusion rules (fail-closed, new reason codes)**: probe-only rail-neutral receipts never export (`probe_only_receipt_excluded`, also caught when the rail-neutral object is passed to the exporter directly); rail-neutral binding candidates must bridge into `reddi.receipt.v1` via the proof chain first (`rail_neutral_bridge_required`); receipts without payment proof refs (dry-run/unproven) are excluded (`payment_proof_missing_excluded`); failure-final receipts (`attestationStatus: failed|rejected`) never export even with a passed attestation (`non_final_receipt_excluded`); pending/not_requested receipts cap at identity metadata with any supplied attestation excluded (`attestation_state_excluded`); non-passed/non-completed attestations (failed/disputed/refunded — negative feedback out of v1 scope) and attestations not referencing the receipt (`attestation_receipt_mismatch`) are excluded. Standalone gate: `evaluateErc8004SourceEligibility(source)`.
- **Composition with `reddi.reputation-credential.v1` (#565/PR #569), reference not duplication**: optional `reputationCredential` option is verified fully offline (ed25519 canonical-body), subject must match the receipt specialist, credential evidence hashes must include the attestation evidence hash (else blocked: `reputation_credential_invalid` / `_subject_mismatch` / `_evidence_mismatch`); success attaches `credentialRef` (id, subject, score, evidence hashes, proof envelope **without** the signature) to the Reputation payload. Guardrails gained `trustImported: false` — one-way boundary (no ERC-8004 trust import into RAP without a separate operator-approved issue) is asserted on every bundle.
- **No-live conformance surface** (`packages/agent-protocol/src/erc8004-export-conformance.ts`, `reddi.erc8004-export-conformance.v1`, exported as `@reddi/agent-protocol/erc8004-export-conformance`): `verifyErc8004ExportAgainstSource(bundle, source)` proves the #562 round-trip — every exported field recomputed from the RAP source and compared (identity/reputation/validation/crossReference/credentialRef/guardrails/no-invented-fields/chain-hint-placeholder checks); any tampering on either side fails a named check; blocked bundles never round-trip. `listErc8004ConformanceFixtures()` + `runErc8004ConformanceSuite()` execute 10 deterministic fixtures covering the full export round-trip, metadata-only, and every exclusion lane. No chain access, no RPC, pure synchronous functions.
- **Spec doc**: `docs/ERC8004-EXPORT-SPEC-2026-07-06.md` — scope, standard-neutral positioning, one-way boundary, chain documentation without deployment claims, per-registry mapping tables, composition contract, exclusion table, conformance contract.
- Tests: `tests/erc8004-export-conformance.test.ts` (12 new node:test tests — suite green, tamper-fail round-trips, credential composition + all three credential fail-closed lanes, probe-only direct-pass exclusion, not_requested exclusion, provenance/unsupported-field coverage, offline/no-async source guard over both modules) and `tests/erc8004-export.test.ts` updated for the promoted shape (draft:false + externalStandard + trustImported guardrail). Barrel + package exports wired, rebuilt dist committed.

Validation:
- `cd packages/agent-protocol && npm test` PASS (421 tests, incl. 12 new; exports-resolution guard in-suite).
- `npm run check:exports` PASS. `npm run check:rap:naming` PASS. `git diff --check` PASS.

Boundary note: no chain/RPC access, no live calls, no EVM writes, no signer, no minting, no live signature verification, no raw-evidence export, no trust import — one-way RAP → ERC-8004 fixtures only. ERC-8004 field shapes remain unverified against the live standard (external draft); no deployment claim is made on any chain.

## Latest Update — Airwallex webhook fixtures → rail-neutral probe-only receipts (2026-07-06 AEST)

Implemented GitHub issue #580 (follow-up 2 of 3 from the #541 Airwallex integration assessment, under rail-neutrality epic #338) in isolated worktree `projects/reddi-agent-protocol-worktree-580-airwallex-webhook-receipts` on branch `feat/580-airwallex-webhook-receipt-normalization`, branched from `origin/main` at `db76f6c6` (main moved today when PR #587 — the #579 rail fixture this issue consumes — and PR #586 merged; branching from the new tip avoids any conflict with them). Touches only `packages/agent-protocol` and this file.

Delivered:
- Added `packages/agent-protocol/src/airwallex-webhook-receipt-normalization.ts` (`reddi.airwallex-webhook-receipt.v1`, exported as `@reddi/agent-protocol/airwallex-webhook-receipt-normalization`) — normalizes STATIC, SYNTHETIC Airwallex webhook event fixtures (payment_intent/payment_link success shapes, every field tagged `(unverified — Airwallex docs)`) into rail-neutral receipt shapes CAPPED at `probe_only` (`airwallex_webhook_receipt_probe_only` in the #579 vocabulary; `railNeutralSupportState: 'probe_only'` in the shared one). Card-rail receipts are revocable, so normalization never produces a final/settled/binding receipt.
- Signature model per the #580 contract: HMAC material is fixture-asserted only — `fixture_asserted: true`, `signatureVerifiedLive: false`, `merchantSecretHeld: false`; no real HMAC is ever computed or verified (no `node:crypto` import; test-enforced). RAP never holds a merchant webhook secret; fixtures claiming live verification or carrying whsec_-shaped / digest-shaped signature values are rejected.
- Fail-closed lanes (`normalizeAirwallexWebhookFixture`, pure/offline): refund/dispute/reversal/cancellation events are explicitly NOT receipts (`revocation_event_not_receipt`, audit notes documenting the revocability gap and routing it to #338); unknown events, malformed envelopes, missing/non-fixture-asserted signatures, credential material, live URLs, PAN/email-shaped PII, custody and settlement-finality claims, and non-synthetic-looking fixtures all rejected into `unsupported_live_airwallex_settlement`. 19 annotated rejection fixtures + `listAirwallexWebhookRejectionFixtures()`.
- Extended `rail-neutral-payment-receipts.ts` input union with `{ rail: 'airwallex-hosted-checkout'; fixture: AirwallexWebhookFixture }` (per the #541 AC sketch): derivation emits `supportState: 'probe_only'` with `bindingIntegration.compatible: false` + `incompatibilityReasons` (fiat outside the Solana-only receipt v1 network table; revocability gap #338; fixture-asserted signature), new `unit: 'fiat-minor-units'`, new error codes `revocable_event_rejected`/`pii_rejected`; the receipt-v1 network gate now applies only to `receipt_binding_candidate` receipts (pay-sh/mpp outputs unchanged; only one demo-count test assertion updated).
- Probe-only cap enforced downstream: `rail-neutral-proof-chain-fixture.ts` now fail-closes any non-`receipt_binding_candidate` receipt before bridging into `reddi.receipt.v1` (new blocked case `airwallex_webhook_probe_only_cap` proves an Airwallex success fixture never mints a receipt v1 envelope, evidence record, or binding).
- RECEIPT-V1 GAP documented (not solved, routes to #338): the frozen `reddi.receipt.v1` union has no revoked/contested state for reversible rails, so a card rail cannot be represented truthfully as final — stated in module docs, recorded on every emitted shape (`revocability.gapTrackedInIssue: 338`, `receiptV1RevocationRepresentable: false`), and the frozen union is deliberately NOT widened.
- Tests: `packages/agent-protocol/tests/airwallex-webhook-receipt-normalization.test.ts` (17 node:test tests) — probe-only cap in both vocabularies, fixture-asserted signature model, #338 gap recording, revocation-family fail-closed sweep (all 7 event names), all rejection lanes, determinism/no-mutation, PII/URL/credential-free valid fixtures, rail-neutral union derivation + error mapping, proof-chain cap, offline/no-async/no-crypto source guard, unverified-tag guard. Wired into barrel + package exports with rebuilt dist committed.

Validation:
- `cd packages/agent-protocol && npm test` PASS (409 tests, incl. 17 new; exports-resolution guard in-suite).
- `npm run check:exports` PASS (83 targets). `npm run check:rap:naming` PASS. `git diff --check` PASS.

Boundary note: no Airwallex account, signup, sandbox/demo credential, API call, webhook registration or delivery, live HMAC verification, merchant-secret custody, partnership contact, spend, or live payment — static synthetic fixtures only. Webhook-derived receipts cap at probe_only; RAP claims no custody, MoR status, money transmission, or settlement finality on this rail. The revoked/contested receipt-state gap is #338 scope; receipt v1 unchanged.

## Latest Update — Airwallex hosted-checkout rail fixture + AIRWALLEX_RAIL_SUPPORT_STATE_MATRIX (2026-07-06 AEST)

Implemented GitHub issue #579 (follow-up 1 of 3 from the #541 Airwallex integration assessment, under rail-neutrality epic #338) in isolated worktree `projects/reddi-agent-protocol-worktree-579-airwallex-rail-fixture` on branch `feat/579-airwallex-rail-fixture`. Mirrors the AP2 precedent (PR #571): new module + tests + barrel/package exports + rebuilt dist, touching only `packages/agent-protocol` and this file.

Delivered:
- Added `packages/agent-protocol/src/airwallex-hosted-checkout-rail.ts` (`reddi.airwallex-hosted-checkout-rail.v1`, exported as `@reddi/agent-protocol/airwallex-hosted-checkout-rail`) — seller-owned Airwallex Payment Link / hosted-payment-page rail modeled as seller-wrapper *metadata only*: `railKind: 'airwallex-hosted-checkout'`, `merchantOfRecord: 'seller'`, `state: 'fixture'`, `webhookReceiptExpected: true`, opaque non-URL `paymentLinkTemplateRef`, `livePaymentApproved`/`custodySupported` hard-false. RAP never touches funds on this rail — same posture as AUDD proof-metadata-only. Fiat currency stays in the draft `reddi.fiat-rail-fixture` namespace; the frozen v1 `SellerWrapperRailConfig` asset union (SOL | USDC | AUDD) and `seller-wrapper-rail-fixtures.ts` / `seller-wrapper-config.ts` are untouched (the fixture reuses the existing seller-wrapper endpoint identity via a separate `fiatRailsDraft` list).
- `AIRWALLEX_RAIL_SUPPORT_STATE_MATRIX` — three #338 rows mirroring `AP2_RAIL_SUPPORT_STATE_MATRIX` shape: `airwallex_hosted_checkout_fixture`, `airwallex_webhook_receipt_probe_only`, `unsupported_live_airwallex_settlement`; every row `draft: true`, `confidence: 'low'`, with claim-boundary strings ("Airwallex is a regulated settlement/acceptance rail owned by the seller, not by RAP", "RAP claims no custody, money transmission, MoR status, or settlement finality from any Airwallex fixture", "Webhook HMAC signatures are fixture-asserted, never verified against a live merchant secret"). The probe_only row documents the receipt-v1 gap: card-rail receipts are revocable (refund/dispute) and receipt v1 cannot represent a later-revoked/contested receipt — tracked as a #338 receipt-semantics gap; webhook normalization itself is follow-up issue 2 of 3.
- Fail-closed validation `validateAirwallexHostedCheckoutRailFixture` (pure, offline): rejects API-key/client-ID/bearer/webhook-secret material, live payment-link URLs (Airwallex hosts anywhere; any http(s) URL in a template ref), PAN- and email-shaped strings (shopper PII), custody and settlement-finality claims (field-shaped and textual), non-seller MoR, non-fixture states, live-payment approval, and unsafe guardrails — all landing in `unsupported_live_airwallex_settlement`. 13 annotated rejection fixtures + all-false-live-flag guardrails object.
- Every Airwallex field shape is tagged `(unverified — Airwallex docs)` (read from public docs only, unverified against a live API). Tests: `packages/agent-protocol/tests/airwallex-hosted-checkout-rail.test.ts` (19 node:test tests) — matrix rows, revocable-receipt caveat, seller-MoR/fixture-state metadata, guardrails, all rejection lanes, malformed fail-closed, determinism/no-mutation, offline/no-async source guard, unverified-tag guard. Wired into the barrel + package exports with rebuilt dist committed.

Validation:
- `cd packages/agent-protocol && npm test` PASS (391 tests, incl. 19 new; exports-resolution guard in-suite).
- `npm run check:exports` PASS (81 targets). `npm run check:rap:naming` PASS. `git diff --check` PASS.

Boundary note: no Airwallex account, signup, sandbox/demo credential, API call, webhook registration, live HMAC verification, partnership contact, spend, or live payment — fixtures only. RAP claims no custody, MoR status, money transmission, or settlement finality on this rail; a live lane requires an explicitly operator-approved boundary re-scope under the #338 gates. No credentials or API surface added.
## Latest Update — Airwallex authorization_controls ↔ BuyerAuthorityPolicy v1 crosswalk (2026-07-06 AEST)

Implemented GitHub issue #581 (docs-only; follow-up 3 of 3 from the #541 Airwallex assessment) in isolated worktree `projects/reddi-agent-protocol-worktree-581-authority-crosswalk` on branch `docs/581-airwallex-authority-crosswalk`. Single new file: `docs/AIRWALLEX-AUTHORIZATION-CONTROLS-BUYER-AUTHORITY-CROSSWALK-2026-07-06.md` — a field-by-field crosswalk mapping Airwallex issuing `authorization_controls` concepts (per-transaction/interval spend limits, merchant/category restrictions, currency rules, expiry, card status, step-up) onto `reddi.buyer-authority-policy.v1` (`packages/agent-protocol/src/buyer-authority-policy.ts`, #548→#549 lineage, PRs #555–#557). Purpose: portability proof that the buyer-authority contract generalizes to a regulated fiat issuing surface with **no schema change and no implementation commitment**.

Key findings recorded: per-transaction cap / expiry / freeze / above-threshold approval map cleanly onto `spendCaps` `per_request` / `expiresAt` / `mode: 'deny'` / `operatorApproval`; partial maps are cumulative daily/monthly windows (declared in schema but the pure evaluator enforces only `per_request`), currency controls (closed `SOL|USDC|AUDD` asset union — the one real schema pressure, per #541 §4.1), merchant-category controls (`sellerAllowlist` is identity-level only), and time windows (single expiry instant). No-equivalent rows: remaining-limit introspection (Airwallex-only); `allowedRails` support states, `receiptEvidence`, `refundFailurePolicy` (RAP-only); custody/live-payment deliberately inverted (`allowLivePayment: false`, `forbidCustody: true`, `forbidSettlementFinality: true`). All gaps routed to #338 as notes — v1 stays frozen. Semantic framing documented: issuing controls are card-side pre-authorization (issuer declines after the attempt); BuyerAuthorityPolicy is agent-side pre-invocation (fail-closed before any request exists) — complementary layers, not equivalents.

Boundary (inherited from #541 verbatim): docs only; no Airwallex account/signup/API call/credential/partnership contact/spend; no proprietary doc reproduction (all Airwallex field names flagged unverified); no custody/MoR/money-transmission/settlement-finality claims. Related: #338, #375; AP2 precedent #563/PR #571; concurrent #579 rail-fixture code work untouched (this branch touches docs/ + STATUS.md only).

Validation: `npm run check:rap:naming` PASS, `git diff --check` PASS.
